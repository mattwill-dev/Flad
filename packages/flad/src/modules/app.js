...
(() => {
  ...
  /* ── Translations ─────────────────────────────────────── */
  const { t } = window.NSXI18n || {};

  // Apply translations to this skin's data-i18n DOM (core's NSXI18n is
  // DOM-free and only provides t()).
  ...
  const canExecuteOperation = (operation, state) =>
    NSXCore.canExecuteOperation(operation, state);
  ...
  function hasActiveFilters() {
    return (
      workflowFilters.roasters.size > 0 ||
  ...
  function getDisplayWorkflows() {
    if (!workflowSearchQuery && !hasActiveFilters())
      return workflowItems.map(_attachRecipeRating);
    ...
    return workflowItems.reduce((acc, w, i) => {
      ...
      if (
        workflowFilters.roasters.size > 0 &&
        !workflowFilters.roasters.has(w.coffeeRoaster)
      )
        return acc;
      if (
        workflowFilters.beans.size > 0 &&
        !workflowFilters.beans.has(w.coffeeName)
      )
        return acc;
      if (
        workflowFilters.grinders.size > 0 &&
        !workflowFilters.grinders.has(w.grinderModel)
      )
        return acc;
      if (
        workflowFilters.profiles.size > 0 &&
        !workflowFilters.profiles.has(w.profileTitle)
      )
        return acc;
  ...
  function buildFilterChips(containerEl, values, activeSet) {
    ...
    allValues.forEach((val) => {
      ...
      btn.type = "button";
      btn.className =
        "filter-chip" + (val === EMPTY_VALUE ? " filter-chip-empty" : "");
  ...
  function openFilterModal() {
    const roasters = [
      ...new Set(workflowItems.map((w) => w.coffeeRoaster)),
    ].sort();
    const beans = [...new Set(workflowItems.map((w) => w.coffeeName))].sort();
    const grinders = [
      ...new Set(workflowItems.map((w) => w.grinderModel)),
    ].sort();
    const profiles = [
      ...new Set(workflowItems.map((w) => w.profileTitle)),
    ].sort();
  ...
  filterChipsRoaster?.addEventListener("click", (e) =>
    handleFilterChipClick(e, workflowFilters.roasters),
  );
  filterChipsBean?.addEventListener("click", (e) =>
    handleFilterChipClick(e, workflowFilters.beans),
  );
  filterChipsGrinder?.addEventListener("click", (e) =>
    handleFilterChipClick(e, workflowFilters.grinders),
  );
  filterChipsProfile?.addEventListener("click", (e) =>
    handleFilterChipClick(e, workflowFilters.profiles),
  );
  ...
  function _openHistoryFilterModal() {
    ...
    buildFilterChips(
      document.getElementById("history-filter-chips-roaster"),
      roasters,
      _historyFilters.roasters,
    );
    buildFilterChips(
      document.getElementById("history-filter-chips-bean"),
      beans,
      _historyFilters.beans,
    );
    buildFilterChips(
      document.getElementById("history-filter-chips-grinder"),
      grinders,
      _historyFilters.grinders,
    );
    buildFilterChips(
      document.getElementById("history-filter-chips-profile"),
      profiles,
      _historyFilters.profiles,
    );
  ...
  document
    ...
    ?.addEventListener("click", _openHistoryFilterModal);

  document
    .getElementById("btn-history-filter-done")
    ?.addEventListener("click", () => {
      historyFilterModalEl.hidden = true;
    });

  document
    .getElementById("btn-history-filter-reset")
    ?.addEventListener("click", () => {
  ...
  document
    .getElementById("history-filter-fav-btn")
    ?.addEventListener("click", () => {
      _historyFilters.favoritesOnly = !_historyFilters.favoritesOnly;
      document
        .getElementById("history-filter-fav-btn")
        ?.classList.toggle("is-active", _historyFilters.favoritesOnly);
      _updateHistoryFilterBtn();
      renderHistory();
    });

  document
    .getElementById("history-filter-rating")
  ...
  document
    .getElementById("history-filter-chips-roaster")
    ?.addEventListener("click", (e) =>
      _handleHistoryChipClick(e, _historyFilters.roasters),
    );
  document
    .getElementById("history-filter-chips-bean")
    ?.addEventListener("click", (e) =>
      _handleHistoryChipClick(e, _historyFilters.beans),
    );
  document
    .getElementById("history-filter-chips-grinder")
    ?.addEventListener("click", (e) =>
      _handleHistoryChipClick(e, _historyFilters.grinders),
    );
  document
    .getElementById("history-filter-chips-profile")
    ?.addEventListener("click", (e) =>
      _handleHistoryChipClick(e, _historyFilters.profiles),
    );
  ...
  const normalizeWorkflowKeyPart = (value) =>
    NSXCore.normalizeWorkflowKeyPart(value);
  ...
  function getShotDetailsCached(shotId) {
    if (!shotId) {
      return Promise.reject(new Error("Invalid shot ID"));
    }
  ...
  const getShotDurationSeconds = (fullShot) =>
    NSXCore.getShotDurationSeconds(fullShot);
  const normalizeShotData = (shot) => NSXCore.normalizeShotData(shot);

  const buildShotDiffData = (
    currentShot,
    latestShot,
    currentDurationSec,
    latestDurationSec,
  ) =>
    NSXCore.buildShotDiffData(
      currentShot,
      latestShot,
      currentDurationSec,
      latestDurationSec,
  ...
  const buildWorkflowItemsFromShots = (shotItems) =>
    NSXCore.buildWorkflowItemsFromShots(shotItems, _recipeRatingCache);
  const findShotsForWorkflow = (workflow) =>
    NSXCore.findShotsForWorkflow(workflow, shots);
  ...
  function _getLiveProfileFrames() {
    ...
    if (title && profileCache) {
      const match = profileCache.find(
        (r) => String(r.profile?.title || "").trim() === title.trim(),
      );
  ...
  const _buildRecipeGatewayPayload = (workflow) =>
    NSXCore.buildGatewayPayload(workflow, { scaleConnected });
  ...
  async function pushSelectedWorkflowToMachine(workflow) {
    ...
    if (!canExecuteOperation("setWorkflow")) {
      showToast(
        t("toast.recipeStateError").replace(
          "{state}",
          NSXCore.getMachineState(),
        ),
      );
    ...
    try {
      ...
      console.warn("Recipe could not be set on machine:", err?.message || err);
      setWorkflowSyncState?.("error");
      showToast(t("toast.recipeNotSet"));
    }
  }
  ...
  async function _pushCurrentSkinStateToMachine(bypassStateCheck = false) {
    ...
    if (workflow) {
      ...
      try {
        ...
        console.warn("Recipe sync failed:", err?.message);
        setWorkflowSyncState?.("error");
      }
      return;
    }
    ...
    try {
      await pushSteamSettings(
        NSXCore.isSteamEnabled() ? NSXCore.getSteamTemp() : 0,
        NSXCore.isSteamEnabled() ? NSXCore.getSteamFlow() : 0,
      );
    } catch (err) {
      console.warn("Steam-Sync fehlgeschlagen:", err?.message);
    }
    try {
      await pushHotwaterSettings(
    ...
    try {
      await pushFlushSettings(
        NSXCore.getFlushFlow(),
  ...
  function renderHomeRecentRecipes() {
    ...
    if (!workflowItems || workflowItems.length === 0) {
      ...
      rowsEl
        .querySelector(".home-rr-row--empty")
    ...
    if (headEl) {
      ...
      headEl.innerHTML =
        "<span></span>" +
        labels
          .map((l) => `<span class="home-rr-header-cell">${l}</span>`)
          .join("");
    }
    ...
    const rowData = top3.map(({ w, i }, rank) => {
      ...
      const syncState =
        sel && existingDot
          ? (["is-synced", "is-pending", "is-error"].find((cls) =>
              existingDot.classList.contains(cls),
            ) ?? "")
          : "";
      const syncDot = sel
        ? `<span class="workflow-sync-dot home-rr-sync-dot${syncState ? " " + syncState : ""}"></span>`
        : "";
      return {
        i,
        rank,
        lastShot,
        html: `<button type="button" class="home-rr-row${sel ? " is-selected" : ""}" data-workflow-index="${i}">
    ...
    for (const { lastShot } of rowData) {
      ...
      getShotDetailsCached(lastShot.id)
        .then((full) => {
          ...
          const span = rowsEl.querySelector(
            `.home-rr-dur[data-rr-shot-id="${lastShot.id}"]`,
          );
  ...
  function plotWorkflowShot(workflow, requestedIndex, _retrying = false) {
    ...
    const workflowChanged =
      !currentNavState || currentNavState.workflowKey !== workflowKey;
    ...
    const rawIndex = Number.isInteger(requestedIndex)
      ? requestedIndex
      : fallbackIndex;
      ...
      fetchShots(filterParams)
        .then((res) => {
          ...
          if (fetched.length > 0) {
            ...
            shots = [
              ...shots,
              ...fetched.filter((s) => !existingIds.has(s.id)),
            ];
          }
        })
        .catch(() => {});
    ...
    Promise.all([
      ...
      .then(([fullShot, latestFullShot]) => {
        ...
        const diffRows = buildShotDiffData(
          shot,
          latestShot,
          currentDurationSec,
          latestDurationSec,
        ...
        const displayWorkflow = isHistorical
          ? mapShotToWorkflow(fullShot)
          : workflow;
      ...
      .catch((err) => {
        ...
        graphEl.innerHTML =
          '<div style="display: flex; align-items: center; justify-content: center; height: 100%; color: var(--c-label-3);">Error loading</div>';
      });
  }
  ...
  const espressoFullscreenGraphEl =
  ...
  function _updateReserveWidget() {
    ...
    const pct =
      targetYield > 0 ? Math.min(100, (weightNow / targetYield) * 100) : 0;

    if (timeEl) timeEl.textContent = `${Math.max(0, elapsed).toFixed(1)}s`;
    if (weightEl)
      weightEl.textContent = `${weightNow.toFixed(1)} / ${targetText} g`;
  ...
  function updateEspressoFullscreen() {
    ...
    _setFsText(
      "espresso-fs-temp",
      Number.isFinite(temp) && temp > 0 ? temp.toFixed(1) : "—",
    );
  ...
  function startLiveShotSession() {
    ...
    requestAnimationFrame(() => {
      if (espressoFullscreenGraphEl)
        initLiveShotChart?.(espressoFullscreenGraphEl);
  ...
  async function endLiveShotSession() {
    ...
    if (_reserve) {
      _reserve.classList.add("is-visible");
    }
    ...
    const _targetVolume = Number(
      _capturedWorkflow?.gatewayWorkflow?.profile?.target_volume ||
        _capturedWorkflow?.profile?.target_volume ||
        0,
    );

    const _wasWeightStop =
      _targetYield > 0 && _capturedWeight >= _targetYield * 0.92;

    let _stopReason;
    if (_wasWeightStop) {
      _stopReason = t("shot.stopWeight").replace(
        "{weight}",
        _capturedWeight.toFixed(1),
      );
    } else if (!_wasDone) {
    ...
    const _hideLiveWidget = () => {
      ...
      document
        .querySelector(".workflow-graph-area")
        ?.classList.remove("is-live");
    };
    ...
    const _runPostShotActions = (newShot) => {
      ...
      if (newShot.id && recipeIdx !== -1) {
        getShotDetailsCached(newShot.id)
          .then((fullShot) => {
            let shotVolume = null;
            let shotWeight = null;
            ...
            if (Array.isArray(meas)) {
              for (let i = meas.length - 1; i >= 0; i--) {
                const w =
                  meas[i]?.scale?.weight ??
                  meas[i]?.scale?.weight_grams ??
                  null;
                if (Number.isFinite(w) && w > 0 && shotWeight === null) {
                  shotWeight = w;
                }
                const v = meas[i]?.machine?.volume ?? meas[i]?.volume ?? null;
                if (Number.isFinite(v) && v > 0 && shotVolume === null) {
                  shotVolume = v;
                }
            ...
            const newSample =
              Number.isFinite(shotVolume) &&
              Number.isFinite(shotWeight) &&
              shotWeight > 0
                ? shotVolume / shotWeight
                : null;
      ...
      if (
        _capturedWeight < 1 &&
        _capturedWorkflow.useVolumeStopWhenNoScale &&
        _capturedVolume > 0 &&
        newShot.id
      ) {
      ...
      if (batchId && dose > 0) {
        fetchBatch(String(batchId))
          .then((batch) => {
            if (batch?.weightRemaining != null) {
              const newWeight = Math.max(
                0,
                Number(batch.weightRemaining) - dose,
              );
              return updateBatch(String(batchId), {
                weightRemaining: newWeight,
              });
            }
          })
    ...
    const applyRefreshedShots = (newShots) => {
      ...
      if (workflowItems[selectedWorkflowIndex]) {
        ...
        selectedWorkflowIndex = Math.max(
          0,
          Math.min(selectedWorkflowIndex, workflowItems.length - 1),
        );
      }
      renderWorkflows(getDisplayWorkflows(), selectedWorkflowIndex);
    ...
    const pollForNewShot = async () => {
      try {
        ...
        const hasNew =
          (Boolean(nextLatestShotId) &&
            nextLatestShotId !== prevLatestShotId) ||
          newShots.length > prevCount;
        if (hasNew) {
          const newIds = new Set(newShots.map((s) => s.id));
          const merged = [
            ...newShots,
            ...shots.filter((s) => !newIds.has(s.id)),
          ];
  ...
  function startSteamSession() {
    ...
    if (cornerRingEl)
      cornerRingEl.style.strokeDashoffset = String(HW_CIRCUMFERENCE);
    ...
    steamTimerInterval = setInterval(() => {
      ...
      if (cornerRingEl && steamDuration > 0) {
        ...
        cornerRingEl.style.strokeDashoffset = String(
          HW_CIRCUMFERENCE * (1 - pct),
        );
      }
  ...
  function startHotWaterSession() {
    ...
    if (targetLabelEl)
      targetLabelEl.textContent =
        NSXCore.getHotwaterVolume() > 0
          ? `/ ${NSXCore.getHotwaterVolume()} ml`
          : "/ — ml";
    if (progressEl)
      progressEl.style.strokeDashoffset = String(HW_CIRCUMFERENCE);
  ...
  function startFlushSession() {
    ...
    _flushTimerInterval = setInterval(() => {
      ...
      if (elapsedEl)
        elapsedEl.textContent = String(
          Math.max(0, flushDuration - Math.floor(elapsed)),
        );
  ...
  async function loadApiData() {
    try {
      ...
      console.warn("Machine info could not be loaded:", e.message);
    }
    ...
    try {
      ...
      if (existing?.id && !NSXCore.getScheduleState().scheduleId) {
        ...
        console.log(
          "[Schedule] Loaded existing schedule id from API:",
          existing.id,
        );
      }
    } catch (e) {
      console.warn(
        "[Schedule] Could not fetch schedules on startup:",
        e.message,
      );
    }
    ...
    try {
      ...
      _shotsTotalCount = Number.isFinite(shotsResult.total)
        ? shotsResult.total
        : shots.length;
  ...
  function signalUserPresence() {
    ...
    if (typeof apiSignalUserPresenceHeartbeat === "function") {
      apiSignalUserPresenceHeartbeat().catch((err) =>
        console.debug("Heartbeat failed:", err.message),
      );
      return;
    }

    fetch(`${GATEWAY}/api/v1/machine/heartbeat`, { method: "POST" }).catch(
      (err) => console.debug("Heartbeat failed:", err.message),
    );
  }
  ...
  async function revalidateOpenViews() {
    if (profilePickerModalEl && !profilePickerModalEl.hidden) {
      await _silentRevalidate(
        NSXCore.getProfiles,
        () => _ensureProfilesLoaded(true),
        _renderProfilePickerList,
      );
    }
    if (muehlenModalEl && !muehlenModalEl.hidden) {
      await _silentRevalidate(NSXCore.getGrinders, NSXCore.loadGrinders, () =>
        renderGrinderTiles(NSXCore.getGrinders()),
      );
    }
    if (beanManagerModalEl && !beanManagerModalEl.hidden) {
      await _silentRevalidate(
        NSXCore.getBeans,
        () => NSXCore.loadBeans(true),
    ...
    try {
      await NSXCore.loadNsxNamespace(true);
    } catch {
      /* keep cache */
    }
    ...
    if (nsAfter && nsAfter !== nsBefore) {
      ...
      if (!workflowItems.some((w) => w.isPending)) {
        _rebuildWorkflowsFromRecipes(
          Array.isArray(nsAfter.recipes) ? nsAfter.recipes : [],
        );
      }
      if (profilePickerModalEl && !profilePickerModalEl.hidden) {
    ...
    try {
      ...
      if (res && res !== _lastShotsResponse) {
        ...
        _shotsTotalCount = Number.isFinite(res.total)
          ? res.total
          : shots.length;
        renderHistory();
      }
  ...
  function _maybeAutoTareNegative(weight) {
    ...
    if (
      ["espresso", "steam", "hotWater", "flush"].includes(
        NSXCore.getMachineState(),
      )
    )
      return;
  ...
  NSXCore.on("scaleWeight", (d) => {
    ...
    const scalePill = document.getElementById("workflow-scale-pill");
    if (scalePill)
      scalePill.textContent = scaleConnected ? newWeight.toFixed(1) + "g" : "";
  ...
  document
    .getElementById("scale-connect-toggle")
  ...
  document
    .getElementById("machine-icon-area")
    ?.addEventListener("click", () => {
  ...
  NSXCore.on("timeToReady", (d) => {
    ...
    const { remainingMs } = d || {};
    const isWarmingUp =
      NSXCore.getMachineState() === "heating" ||
      NSXCore.getMachineState() === "preheating";
    if (isWarmingUp && typeof remainingMs === "number" && remainingMs > 0) {
      readyInChipEl.textContent = t("machine.readyIn").replace(
        "{time}",
  ...
  NSXCore.on("machineState", (d) => {
    ...
    if (state === "espresso" && !wasEspressoLike) {
      ...
      if (_reserve) {
        _reserve.classList.add("is-visible");
        _reserve.classList.add("shot-active");
      }
  ...
  NSXCore.on("liveShot", (snap) => {
    ...
    if (steamSession && snap?.state?.state === "steam") {
      ...
      if (pEl)
        pEl.textContent = Number.isFinite(snapPressure)
          ? snapPressure.toFixed(1)
          : "—";
      if (fEl)
        fEl.textContent = Number.isFinite(snapFlow) ? snapFlow.toFixed(1) : "—";
      if (tEl)
        tEl.textContent = Number.isFinite(snapTemp)
          ? Math.round(snapTemp).toString()
          : "—";
    }
    ...
    if (liveShot && snap?.state?.state === "espresso") {
      ...
      if (substate === "preinfusion" || substate === "pouring") {
        ...
        const t =
          (new Date(snap.timestamp).getTime() - liveShot.dataStart) / 1000;
        ...
        if (
          Number.isFinite(profileFrame) &&
          profileFrame !== liveShot.lastProfileFrame
        ) {
          const frameDef = frames[profileFrame] ?? null;
          const frameLabel = String(
            frameDef?.name || `Step ${profileFrame + 1}`,
          );
          liveShot.phaseMarkers.push({
            time: Math.max(0, t),
            label: frameLabel,
          });
          ...
          if (
            _skipStepGuardFrame !== null &&
            profileFrame !== _skipStepGuardFrame
          ) {
            _skipStepGuardFrame = null;
          }
        ...
        liveShot.targetPressure.push(
          snap.targetPressure > 0 ? snap.targetPressure : null,
        );
        ...
        liveShot.targetTemperature.push(
  ...
  recipeListScrollEl.addEventListener("scroll", updateRecipeListFade, {
    passive: true,
  });
  ...
  function _normalizePresenceTimeout(minutes) {
    ...
    if (!Number.isFinite(numericMinutes))
      return SKIN_DEFAULTS.sleepTimeoutMinutes;
  ...
  function _applyScale(setting) {
    ...
    const autoWidthModalIds = new Set([
      "steam-corner",
      "flush-overlay",
      "hotwater-overlay",
      "needswater-overlay",
  ...
  window.NSXSkinControls = {
    ...
    getScreensaverDimEnabled: () =>
      storeSettings.nsx_screensaver_dim_enabled !== false,
    ...
    setScreensaverBrightness(v) {
      ...
      patchStoreSettings({
        nsx_screensaver_brightness: Number.isFinite(n) ? n : 50,
      });
      _pushScreensaverConfig();
    },
    getScreensaverImages: () =>
      window.NSXScreensaver?.getCustomImages?.() || [],
    getScreensaverMaxImages: () => window.NSXScreensaver?.maxCustomImages ?? 20,
    addScreensaverImages: (files) =>
      window.NSXScreensaver?.addCustomImages?.(files),
    removeScreensaverImage: (i) =>
      window.NSXScreensaver?.removeCustomImage?.(i),
    ...
    getShowRecipeCardRating: () =>
      storeSettings.nsx_show_recipe_card_rating !== false,
    ...
    measureDosingCup() {
      ...
      if (!Number.isFinite(w) || w <= 0) {
        showToast("Place the empty cup on the scale first");
        return null;
      }
  ...
  document
    .getElementById("btn-home-profile-picker")
    ?.addEventListener("click", () => {
  ...
  /* ── Reinigung Flow ─────────────────────────────────── */
  {
    ...
    const cleaningProfileListEl = document.getElementById(
    ...
    const cleaningProgressTextEl = document.getElementById(
      "cleaning-progress-text",
    );
    const cleaningStep3TitleEl = document.getElementById(
      "cleaning-step3-title",
    );

    // "Step x of y": y = number of frames in the cleaning profile,
    ...
    document
      .getElementById("btn-cleaning-skip-step")
      ?.addEventListener("click", () => {
        ...
        const currentFrame = Number.isFinite(liveShot?.lastProfileFrame)
    ...
    async function _findCleaningShotCandidate() {
      ...
      const newSinceStart = items
        .filter((shot) => {
          ...
          if (runStartMs > 0 && ts > 0 && ts < runStartMs - 10_000)
            return false;
          return true;
        })
    ...
    function _cleaningStopGraph() {
      ...
      if (cleaningStep3TitleEl)
        cleaningStep3TitleEl.textContent = t("cleaning.title");
    }
    ...
    function _cleaningClose() {
      ...
      if (_cleaningStateHandler) {
        window.removeEventListener(
          "gateway:machineState",
          _cleaningStateHandler,
        );
    ...
    document
      .getElementById("btn-home-cleaning")
      ?.addEventListener("click", () => {
        _cleaningShowStep(1);
    ...
    document
      .getElementById("btn-cleaning-cancel")
      ?.addEventListener("click", _cleaningClose);
    document
      .getElementById("btn-cleaning-step3-abort")
      ?.addEventListener("click", _cleaningClose);
    document
      .getElementById("btn-cleaning-step2-back")
      ?.addEventListener("click", () => _cleaningShowStep(1));

    // Step 1 → 2: load and filter profiles
    document
      .getElementById("btn-cleaning-ready")
    ...
    // Step 2 → 3: push workflow, monitor machine
    async function _cleaningSelectProfile(record) {
      try {
        ...
        setCurrentWorkflow?.(
          mapApiWorkflowToDisplay(cleaningWorkflow.gatewayWorkflow),
        );
      ...
      // Wait for the espresso cycle: espresso → idle = done
      _cleaningWasEspresso = false;
      _cleaningStateHandler = ({ detail }) => {
        ...
        if (isRunningState && !_cleaningWasEspresso) {
          // Machine running — hide instructions, show graph
          ...
          if (cleaningStep3TitleEl)
            cleaningStep3TitleEl.textContent = t("toast.cleaningRunning");

          requestAnimationFrame(() => {
            initCleaningChart?.(cleaningGraphEl);
            _updateCleaningProgress();
            // Snapshot updates for the cleaning graph
            _cleaningSnapshotHandler = () => {
              if (liveShot) {
    ...
    // Step 4: Done — keep shot, back to home
    document
      .getElementById("btn-cleaning-done")
      ?.addEventListener("click", async () => {
        try {
          await _refreshShotsFromApi(80);
        } catch (err) {
          console.warn(
            "Shots could not be refreshed after cleaning:",
            err.message,
          );
        }
        _cleaningClose();
  ...
  document
    .querySelector(".workflow-scale-indicator")
  ...
  /* ── Recipe Preset Row (recipes screen: 4 quick-select slots) ──────────
   * Slot assignment (which recipe id sits in which of the 4 tiles) is
   * Flad-specific display state, not portable recipe data — kept as a skin
   * setting (nsx_recipe_preset_ids), not on the recipe record itself. */

  const RECIPE_PRESET_SLOT_COUNT = 4;
  let _pickerTargetSlot = null;

  function _getPresetSlotIds() {
    const raw = Array.isArray(storeSettings.nsx_recipe_preset_ids)
      ? storeSettings.nsx_recipe_preset_ids
      : [];
  ...
  function _assignPresetSlot(slotIndex, recipeId) {
    const ids = _getPresetSlotIds();
    ids[slotIndex] = recipeId || null;
    patchStoreSettings({ nsx_recipe_preset_ids: ids });
    renderRecipePresetRow();
  }

  // Auto-slot a newly created recipe into the first empty preset slot. If all
  // 4 are already taken, leave the recipe unassigned (still reachable via the
  // picker) rather than bumping an existing preset — user picks what to swap.
  function _autoAssignPresetSlot(recipeId) {
    const ids = _getPresetSlotIds();
    const emptySlot = ids.indexOf(null);
    if (emptySlot === -1) {
      showAlert(t("recipe.presetsFullMessage"));
      return;
    }
    ids[emptySlot] = recipeId;
    patchStoreSettings({ nsx_recipe_preset_ids: ids });
  }

  function renderRecipePresetRow() {
    const rowEl = document.getElementById("recipe-preset-row");
    if (!rowEl) return;
    rowEl.innerHTML = _getPresetSlotIds()
      .map((id, slot) => {
        const idx = id ? workflowItems.findIndex((w) => w.id === id) : -1;
        const workflow = idx >= 0 ? workflowItems[idx] : null;
        if (!workflow) {
          return `<li class="recipe-preset-tile recipe-preset-tile--empty" data-slot="${slot}" role="button" tabindex="0" aria-label="${t("recipe.presetEmptyAria")}">
        <span class="recipe-preset-empty-icon" aria-hidden="true">+</span>
        <span class="recipe-preset-empty-label">${t("recipe.presetEmpty")}</span>
      </li>`;
        }
        const isActive = idx === selectedWorkflowIndex;
        const bean =
          [workflow.coffeeRoaster, workflow.coffeeName]
            .filter((v) => v && v !== "—")
            .join(" · ") || "—";
        const doseYield = `${workflow.targetDoseWeight}g:${workflow.targetYield}g`;
        return `<li class="recipe-preset-tile${isActive ? " is-active" : ""}" data-slot="${slot}" data-workflow-index="${idx}" role="button" tabindex="0" aria-label="${t("recipe.presetSwapAria")}">
      <span class="recipe-preset-tile-bean">${workflow.profileTitle || "—"}</span>
      <span class="recipe-preset-tile-meta">${doseYield} · ${bean}</span>
    </li>`;
      })
      .join("");
  }
  ...
  function openRecipePickerModal(slotIndex) {
    ...
    renderWorkflows(getDisplayWorkflows(), selectedWorkflowIndex);
    const modalEl = document.getElementById("recipe-picker-modal");
  ...
  function closeRecipePickerModal() {
    ...
    _pickerTargetSlot = null;
  }

  const recipePresetRowEl = document.getElementById("recipe-preset-row");
  let _presetLongPressTimer = null;
  let _presetLongPressFired = false;

  recipePresetRowEl?.addEventListener("pointerdown", (event) => {
    const tile = event.target.closest(".recipe-preset-tile");
    if (!tile) return;
    _presetLongPressFired = false;
    clearTimeout(_presetLongPressTimer);
    _presetLongPressTimer = setTimeout(() => {
      _presetLongPressFired = true;
      openRecipePickerModal(Number(tile.dataset.slot));
    }, 500);
  });

  const _cancelPresetLongPress = () => clearTimeout(_presetLongPressTimer);
  recipePresetRowEl?.addEventListener("pointerup", _cancelPresetLongPress);
  recipePresetRowEl?.addEventListener("pointerleave", _cancelPresetLongPress);
  recipePresetRowEl?.addEventListener("pointercancel", _cancelPresetLongPress);

  recipePresetRowEl?.addEventListener("click", (event) => {
    if (_presetLongPressFired) {
      _presetLongPressFired = false;
      return;
    }
    const tile = event.target.closest(".recipe-preset-tile");
    if (!tile) return;
    if (tile.dataset.workflowIndex === undefined) {
      openRecipePickerModal(Number(tile.dataset.slot));
      return;
    }
    selectWorkflow(Number(tile.dataset.workflowIndex));
  });
  ...
  document
    ...
    ?.addEventListener("click", (event) => {
      if (event.target === document.getElementById("recipe-picker-modal"))
        closeRecipePickerModal();
    });
  ...
  function _updateSteamPresetButtons() {
    ...
    document
      .querySelectorAll(".steam-card .steam-preset-btn")
      .forEach((btn) => {
  ...
  function _renderSteamPurgeToggle(mode) {
    document
      .querySelectorAll("#steam-purge-segment .grinder-toggle-btn")
      .forEach((btn) => {
  ...
  steamSettingsModalEl
    ...
    .forEach((presetEl) => {
      ...
      presetEl
        .querySelectorAll(".steam-settings-up, .steam-settings-down")
        .forEach((btn) => {
          btn.addEventListener("click", () => {
            const p = _steamSettingsDraft?.[key];
            if (!p) return;
            ...
            if (field === "temp")
              p.temp = Math.min(165, Math.max(100, p.temp + dir * 5));
            if (field === "flow")
              p.flow =
                Math.round(
                  Math.min(4.0, Math.max(0.3, p.flow + dir * 0.1)) * 10,
                ) / 10;
            if (field === "duration")
              p.duration = Math.min(300, Math.max(1, p.duration + dir * 1));
  ...
  steamSettingsModalEl
    ?.querySelectorAll(".steam-settings-name-input")
    .forEach((input) => {
      input.addEventListener("click", () => {
        window._openTextPicker(input.value, (val) => {
          input.value = val || input.placeholder;
        });
      });
    });
  ...
  steamSettingsModalEl
    ...
    .forEach((span) => {
      span.addEventListener("click", () => {
        ...
        const field = span.dataset.field;
        if (field === "temp")
          openNumberPicker(_npMakeRange(100, 165, 5), p.temp, (v) => {
            p.temp = v;
            _renderSteamSettingsValues(presetEl, p);
          });
        if (field === "flow")
          openNumberPicker(
            _npMakeRange(0.3, 4.0, 0.1),
            p.flow,
            (v) => {
              p.flow = v;
              _renderSteamSettingsValues(presetEl, p);
        ...
        if (field === "duration")
          openNumberPicker(_npMakeRange(1, 300, 1), p.duration, (v) => {
            p.duration = v;
            _renderSteamSettingsValues(presetEl, p);
          });
      });
      ...
      steamSettingsModalEl
        ...
        .forEach((el) => {
          ...
          if (_steamSettingsDraft[key]) {
            _steamSettingsDraft[key].name =
              el.querySelector(".steam-settings-name-input").value.trim() ||
              key;
          }
        });
  ...
  document
    .getElementById("steam-power-toggle")
    ?.addEventListener("change", (e) => {
      NSXCore.setSteamEnabled(e.target.checked);
    });

  document
    .getElementById("btn-steam-temp-up")
    ?.addEventListener("click", () =>
      NSXCore.setSteamTemp(NSXCore.getSteamTemp() + 5),
    );
  document
    .getElementById("btn-steam-temp-down")
    ?.addEventListener("click", () =>
      NSXCore.setSteamTemp(NSXCore.getSteamTemp() - 5),
    );
  document
    .getElementById("btn-steam-flow-up")
    ?.addEventListener("click", () =>
      NSXCore.setSteamFlow(NSXCore.getSteamFlow() + 0.1),
    );
  document
    .getElementById("btn-steam-flow-down")
    ?.addEventListener("click", () =>
      NSXCore.setSteamFlow(NSXCore.getSteamFlow() - 0.1),
    );
  document
    .getElementById("btn-steam-dur-up")
    ?.addEventListener("click", () =>
      NSXCore.setSteamDuration(NSXCore.getSteamDuration() + 1),
    );
  document
    .getElementById("btn-steam-dur-down")
    ?.addEventListener("click", () =>
      NSXCore.setSteamDuration(NSXCore.getSteamDuration() - 1),
    );
  ...
  function _updateHotwaterPresetButtons() {
    ...
    document
      .querySelectorAll(".hotwater-card .steam-preset-btn")
      .forEach((btn) => {
        const key = btn.dataset.preset;
  ...
  document
    ...
    .forEach((btn) => {
      btn.addEventListener("click", () =>
        NSXCore.selectHotwaterPreset(btn.dataset.preset),
      );
    });

  document
    .getElementById("btn-hotwater-temp-up")
    ?.addEventListener("click", () =>
      NSXCore.setHotwaterTemp(NSXCore.getHotwaterTemp() + 5),
    );
  document
    .getElementById("btn-hotwater-temp-down")
    ?.addEventListener("click", () =>
      NSXCore.setHotwaterTemp(NSXCore.getHotwaterTemp() - 5),
    );
  document
    .getElementById("btn-hotwater-flow-up")
    ?.addEventListener("click", () =>
      NSXCore.setHotwaterFlow(NSXCore.getHotwaterFlow() + 1.0),
    );
  document
    .getElementById("btn-hotwater-flow-down")
    ?.addEventListener("click", () =>
      NSXCore.setHotwaterFlow(NSXCore.getHotwaterFlow() - 1.0),
    );
  document
    .getElementById("btn-hotwater-vol-up")
    ?.addEventListener("click", () =>
      NSXCore.setHotwaterVolume(NSXCore.getHotwaterVolume() + 10),
    );
  document
    .getElementById("btn-hotwater-vol-down")
    ?.addEventListener("click", () =>
      NSXCore.setHotwaterVolume(NSXCore.getHotwaterVolume() - 10),
    );
  ...
  const hotwaterSettingsModalEl = document.getElementById(
    "hotwater-settings-modal",
  );
  let _hotwaterSettingsDraft = null;

  function _openHotwaterSettingsModal() {
    _hotwaterSettingsDraft = JSON.parse(
      JSON.stringify(NSXCore.getHotwaterPresets()),
    );
    const presetEls =
      hotwaterSettingsModalEl?.querySelectorAll(".hotwater-settings-preset") ??
      [];
    presetEls.forEach((el) => {
      const key = el.dataset.preset;
  ...
  hotwaterSettingsModalEl
    ?.querySelectorAll(".hotwater-settings-preset")
    .forEach((presetEl) => {
      const key = presetEl.dataset.preset;
      presetEl
        .querySelectorAll(".hotwater-settings-up, .hotwater-settings-down")
        .forEach((btn) => {
          btn.addEventListener("click", () => {
            const p = _hotwaterSettingsDraft?.[key];
            if (!p) return;
            ...
            if (field === "temp")
              p.temp = Math.min(100, Math.max(50, p.temp + dir * 5));
            if (field === "flow")
              p.flow =
                Math.round(
                  Math.min(10.0, Math.max(0.5, p.flow + dir * 0.5)) * 10,
                ) / 10;
            if (field === "volume")
              p.volume = Math.min(500, Math.max(10, p.volume + dir * 10));
  ...
  hotwaterSettingsModalEl
    ?.querySelectorAll(".hotwater-settings-value")
    .forEach((span) => {
      span.addEventListener("click", () => {
        ...
        const field = span.dataset.field;
        if (field === "temp")
          openNumberPicker(_npMakeRange(50, 100, 5), p.temp, (v) => {
            p.temp = v;
            _renderHotwaterSettingsValues(presetEl, p);
          });
        if (field === "flow")
          openNumberPicker(
            _npMakeRange(0.5, 10.0, 0.5),
            p.flow,
            (v) => {
              p.flow = v;
              _renderHotwaterSettingsValues(presetEl, p);
        ...
        if (field === "volume")
          openNumberPicker(_npMakeRange(10, 500, 10), p.volume, (v) => {
            p.volume = v;
            _renderHotwaterSettingsValues(presetEl, p);
          });
      });
  ...
  document
    .getElementById("btn-hotwater-settings")
    ?.addEventListener("click", _openHotwaterSettingsModal);

  document
    .getElementById("btn-hotwater-settings-cancel")
    ?.addEventListener("click", () => {
      if (hotwaterSettingsModalEl) hotwaterSettingsModalEl.hidden = true;
    });

  document
    .getElementById("btn-hotwater-settings-save")
    ?.addEventListener("click", () => {
      ...
      hotwaterSettingsModalEl
        ...
        .forEach((el) => {
          ...
          const nameVal = el
            .querySelector(".steam-settings-name-input")
            ?.value.trim();
  ...
  function _updateFlushPresetButtons() {
    ...
    document
      .querySelectorAll(".cleaning-card .steam-preset-btn")
      .forEach((btn) => {
        const key = btn.dataset.preset;
        btn.textContent = presets[key]?.name ?? key;
        btn.classList.toggle("is-active", key === active);
  ...
  document
    ...
    .forEach((btn) => {
      btn.addEventListener("click", () =>
        NSXCore.selectFlushPreset(btn.dataset.preset),
      );
    });

  document
    .getElementById("btn-flush-flow-up")
    ?.addEventListener("click", () =>
      NSXCore.setFlushFlow(NSXCore.getFlushFlow() + 1),
    );
  document
    .getElementById("btn-flush-flow-down")
    ?.addEventListener("click", () =>
      NSXCore.setFlushFlow(NSXCore.getFlushFlow() - 1),
    );
  document
    .getElementById("btn-flush-duration-up")
    ?.addEventListener("click", () =>
      NSXCore.setFlushDuration(NSXCore.getFlushDuration() + 1),
    );
  document
    .getElementById("btn-flush-duration-down")
    ?.addEventListener("click", () =>
      NSXCore.setFlushDuration(NSXCore.getFlushDuration() - 1),
    );
  ...
  function _openFlushSettingsModal() {
    ...
    flushSettingsModalEl
      ?.querySelectorAll(".flush-settings-preset")
      .forEach((el) => {
        const key = el.dataset.preset;
    ...
    fetchMachineSettings?.()
      .then((s) => {
        ...
        if (s?.flushTimeout != null)
          _flushMachineTimeout = Number(s.flushTimeout);
  ...
  flushSettingsModalEl
    ...
    .forEach((presetEl) => {
      const key = presetEl.dataset.preset;
      presetEl
        .querySelectorAll(".flush-settings-up, .flush-settings-down")
        .forEach((btn) => {
          btn.addEventListener("click", () => {
            const p = _flushSettingsDraft?.[key];
            if (!p) return;
            ...
            if (field === "flow")
              p.flow = Math.min(10, Math.max(1, p.flow + dir));
            if (field === "duration")
              p.duration = Math.min(60, Math.max(1, p.duration + dir));
      ...
      presetEl
        .querySelector(".steam-settings-name-input")
        ?.addEventListener("click", function () {
          window._openTextPicker(this.value, (val) => {
      ...
      presetEl.querySelectorAll(".flush-settings-value").forEach((span) => {
        span.addEventListener("click", () => {
          ...
          const field = span.dataset.field;
          if (field === "flow")
            openNumberPicker(_npMakeRange(1, 10, 1), p.flow, (v) => {
              p.flow = v;
              _renderFlushSettingsValues(presetEl, p);
            });
          if (field === "duration")
            openNumberPicker(_npMakeRange(1, 60, 1), p.duration, (v) => {
              p.duration = v;
              _renderFlushSettingsValues(presetEl, p);
            });
        });
  ...
  document
    .getElementById("btn-flush-temp-up")
    ?.addEventListener("click", () => {
      _flushMachineTemp = Math.min(100, _flushMachineTemp + 1);
      _renderFlushMachineSettings();
    });
  document
    .getElementById("btn-flush-temp-down")
    ?.addEventListener("click", () => {
      _flushMachineTemp = Math.max(0, _flushMachineTemp - 1);
      _renderFlushMachineSettings();
    });
  document
    .getElementById("flush-settings-temp")
    ?.addEventListener("click", () => {
      openNumberPicker(_npMakeRange(0, 100, 1), _flushMachineTemp, (v) => {
        _flushMachineTemp = v;
        _renderFlushMachineSettings();
      });
    });
  document
    .getElementById("btn-flush-timeout-up")
    ?.addEventListener("click", () => {
      _flushMachineTimeout = Math.min(120, _flushMachineTimeout + 1);
      _renderFlushMachineSettings();
    });
  document
    .getElementById("btn-flush-timeout-down")
    ?.addEventListener("click", () => {
      _flushMachineTimeout = Math.max(0, _flushMachineTimeout - 1);
      _renderFlushMachineSettings();
    });
  document
    .getElementById("flush-settings-timeout")
    ?.addEventListener("click", () => {
      openNumberPicker(_npMakeRange(0, 120, 1), _flushMachineTimeout, (v) => {
        _flushMachineTimeout = v;
        _renderFlushMachineSettings();
      });
    });

  document
    .getElementById("btn-flush-settings")
    ?.addEventListener("click", _openFlushSettingsModal);

  document
    .getElementById("btn-flush-settings-cancel")
    ?.addEventListener("click", () => {
      if (flushSettingsModalEl) flushSettingsModalEl.hidden = true;
    });

  document
    .getElementById("btn-flush-settings-save")
    ?.addEventListener("click", () => {
      ...
      updateMachineSettings?.({
        flushTemp: _flushMachineTemp,
        flushTimeout: _flushMachineTimeout,
      }).catch(() => {});
      if (flushSettingsModalEl) flushSettingsModalEl.hidden = true;
    });
  ...
  async function hydrateUiSettingsFromStore() {
    try {
      ...
      if (Number.isFinite(Number(storeSettings.nsx_sleep_timeout_minutes))) {
        _presenceTimeoutMinutes = _normalizePresenceTimeout(
          storeSettings.nsx_sleep_timeout_minutes,
        );
      }

      if (Number.isFinite(Number(storeSettings.nsx_display_brightness))) {
        _skinBrightness = _normalizeBrightness(
          storeSettings.nsx_display_brightness,
        );
      ...
      if (
        storeSettings.nsx_series_visibility &&
        typeof storeSettings.nsx_series_visibility === "object"
      ) {
        window.NSXUI?.setSeriesVisibility(
          "workflow",
          storeSettings.nsx_series_visibility,
        );
      }
      if (
        storeSettings.nsx_series_visibility_history &&
        typeof storeSettings.nsx_series_visibility_history === "object"
      ) {
        window.NSXUI?.setSeriesVisibility(
          "history",
          storeSettings.nsx_series_visibility_history,
      ...
      _restoreCollapsedProfileGroups(
        storeSettings.nsx_profile_picker_collapsed_groups,
      );

      if (Array.isArray(storeSettings.nsx_bean_manager_collapsed_roasters)) {
        _beanManagerCollapsedRoasters = new Set(
          storeSettings.nsx_bean_manager_collapsed_roasters,
        );
      }
  ...
  function renderScheduleUI() {
    ...
    document.querySelectorAll(".schedule-day-btn").forEach((btn) => {
      btn.classList.toggle(
        "is-active",
        s.days.includes(Number(btn.dataset.day)),
      );
    });
  ...
  document
    .getElementById("schedule-enabled")
    ?.addEventListener("change", (e) => {
  ...
  document.getElementById("btn-sch-on-h-up")?.addEventListener("click", () =>
    NSXCore.applySchedule({
      onHour: (NSXCore.getScheduleState().onHour + 1) % 24,
    }),
  );
  document.getElementById("btn-sch-on-h-down")?.addEventListener("click", () =>
    NSXCore.applySchedule({
      onHour: (NSXCore.getScheduleState().onHour + 23) % 24,
    }),
  );
  document.getElementById("btn-sch-on-m-up")?.addEventListener("click", () =>
    NSXCore.applySchedule({
      onMinute: (NSXCore.getScheduleState().onMinute + 15) % 60,
    }),
  );
  document.getElementById("btn-sch-on-m-down")?.addEventListener("click", () =>
    NSXCore.applySchedule({
      onMinute: (NSXCore.getScheduleState().onMinute + 45) % 60,
    }),
  );
  ...
  function closeAllSwipes(except = null) {
    workflowListEl
      .querySelectorAll(".workflow-card.swipe-open")
      .forEach((el) => {
        if (el === except) return;
  ...
  workflowListEl.addEventListener(
    "touchstart",
    (e) => {
  ...
  workflowListEl.addEventListener(
    "touchend",
    (e) => {
      if (!swipeState || !swipeState.isHorizontal) {
        swipeState = null;
        return;
      }
  ...
  function closeAllHistorySwipes(except = null) {
    historyListEl
      ?.querySelectorAll(".history-accordion-item.swipe-open")
      .forEach((el) => {
        if (el === except) return;
        el.classList.remove("swipe-open");
  ...
  historyListEl?.addEventListener(
    ...
    (e) => {
      ...
      if (historySwipeState.isHorizontal === null) {
        ...
        if (!historySwipeState.isHorizontal) {
  ...
  historyListEl?.addEventListener(
    "touchend",
    (e) => {
      if (!historySwipeState || !historySwipeState.isHorizontal) {
        historySwipeState = null;
        return;
      }
      e.stopPropagation();
      ...
      layer.style.transition = "";
      const shouldOpen = historySwipeState.wasOpen
        ? dx < THRESHOLD
        : dx < -THRESHOLD;
      if (shouldOpen) {
  ...
  async function deleteWorkflowShots(workflowIndex) {
    ...
    const presetIds = _getPresetSlotIds();
    if (presetIds.includes(workflow.id)) {
      patchStoreSettings({
        nsx_recipe_preset_ids: presetIds.map((id) =>
          id === workflow.id ? null : id,
        ),
      });
    }
    selectedWorkflowIndex = Math.max(
      0,
      Math.min(selectedWorkflowIndex, Math.max(0, workflowItems.length - 1)),
    );
    historySelectedRecipeIndex = Math.min(
      historySelectedRecipeIndex,
  ...
  function _hasActiveHistoryFilters() {
    return (
      _historyFilters.roasters.size > 0 ||
      _historyFilters.beans.size > 0 ||
      _historyFilters.grinders.size > 0 ||
      _historyFilters.profiles.size > 0 ||
      _historyFilters.favoritesOnly ||
      _historyFilters.minRating > 0
    );
  }
  ...
  function _getFilteredHistoryRecipes(all) {
    ...
    return all.filter((w) => {
      ...
      if (
        _historyFilters.roasters.size > 0 &&
        !_historyFilters.roasters.has(w.coffeeRoaster)
      )
        return false;
      if (
        _historyFilters.beans.size > 0 &&
        !_historyFilters.beans.has(w.coffeeName)
      )
        return false;
      if (
        _historyFilters.grinders.size > 0 &&
        !_historyFilters.grinders.has(w.grinderModel)
      )
        return false;
      if (
        _historyFilters.profiles.size > 0 &&
        !_historyFilters.profiles.has(w.profileTitle)
      )
        return false;
      return true;
    });
  ...
  async function _loadMoreHistory() {
    ...
    try {
      ...
      if (newItems.length > 0) {
        ...
        if (useShotsBuffer) shots = merged;
        else historyShots = merged;
      }
    } catch (err) {
      console.warn("More shots could not be loaded:", err?.message);
    } finally {
  ...
  function _loadRecipeRatings(recipes) {
    ...
    const runQueue = async () => {
      for (const r of pending) {
        ...
        try {
          const params = {};
          if (r.coffeeName && r.coffeeName !== "—")
            params.coffeeName = r.coffeeName;
          if (r.coffeeRoaster && r.coffeeRoaster !== "—")
            params.coffeeRoaster = r.coffeeRoaster;
          if (r.grinderModel && r.grinderModel !== "—")
            params.grinderModel = r.grinderModel;
          if (r.profileTitle && r.profileTitle !== "—")
            params.profileTitle = r.profileTitle;
          const items = await _fetchAllRecipeShots(params);
          const matched = items.filter(
            (s) => getWorkflowKey(mapShotToWorkflow(s)) === key,
          );
          const { max, count } = _computeMaxRating(matched);
          _recipeRatingCache.set(key, { max, count });
          updateRecipeRating?.(key, max, count);
        } catch {
          /* ignore per-recipe failures */
        }
      }
  ...
  function renderHistory() {
    ...
    if (
      historySelectedRecipeIndex >= 0 &&
      historySelectedRecipeIndex < filtered.length
    ) {
      selectedShots = findShotsForHistoryWorkflow(
        filtered[historySelectedRecipeIndex],
        source,
      );
    }
  ...
  function _loadHistoryShotDurations(recipeShots) {
    for (const shot of recipeShots) {
      ...
      getShotDetailsCached(shot.id)
        .then((full) =>
          updateHistoryShotDuration?.(shot.id, getShotDurationSeconds(full)),
        )
  ...
  document
    .getElementById("btn-history-load-more")
    ?.addEventListener("click", () => _loadMoreHistory());

  function _updateHistoryViewToggleLabel() {
    const label = document.getElementById("history-view-toggle-label");
    if (label) {
      label.textContent = t(
        _historyViewMode === "shots"
          ? "history.orderedByDate"
          : "history.orderedByRecipe",
      );
    }
  }
  ...
  document
    .getElementById("history-accordion-list")
    ?.addEventListener("click", (e) => {
      ...
      if (deleteAllBtn) {
        ...
        (async () => {
          if (
            !(await showConfirm(
              t("confirm.deleteRecipeShots").replace(
                "{count}",
                recipeShotIds.length,
              ),
            ))
          ) {
            closeAllHistorySwipes();
          ...
          try {
            await Promise.all(
              recipeShotIds.map((id) => NSXCore.deleteShot(id)),
            );
            shots = shots.filter((s) => !recipeShotIds.includes(s.id));
            historyShots = historyShots.filter(
              (s) => !recipeShotIds.includes(s.id),
            );
            _shotsTotalCount = Math.max(
              0,
              _shotsTotalCount - recipeShotIds.length,
            );
            ...
            renderWorkflows(getDisplayWorkflows(), selectedWorkflowIndex);
            showToast(
              t("toast.shotsDeleted").replace("{count}", recipeShotIds.length),
            );
          } catch {
      ...
      if (header) {
        ...
        historySelectedRecipeIndex =
          historySelectedRecipeIndex === idx ? -1 : idx;
        renderHistory();
      }
    });
  ...
  function _clearDoseScaleState() {
    ...
    if (doseYieldEl) {
      ...
      if (wf)
        doseYieldEl.textContent = `${wf.targetDoseWeight}g:${wf.targetYield}g`;
    }
  }
  ...
  function _applyDoseScale() {
    ...
    if (!scaleConnected) {
      showAlert("Scale is not connected.");
      return;
    }
    ...
    pushWorkflow({
      context: { targetDoseWeight: _scaledDose, targetYield: _scaledYield },
    }).catch(() => {});
  }
  ...
  document.getElementById("btn-dose-scale")?.addEventListener("click", () => {
    if (_scaledDose !== null) {
      ...
      if (wf)
        pushWorkflow({
          context: {
            targetDoseWeight: Number(wf.targetDoseWeight || 0),
            targetYield: Number(wf.targetYield || 0),
          },
        }).catch(() => {});
    } else {
      _applyDoseScale();
  ...
  const shotReviewRecipeGridEl = document.getElementById(
    "shot-review-recipe-grid",
  );
  ...
  function _updateReferenceBtn() {
    document
      .getElementById("btn-shot-review-reference")
  ...
  function _rerenderReviewGraph() {
    ...
    shotReviewGraphEl._referenceNormalized =
      _reviewReferenceActive &&
      _reviewReferenceFull &&
      _reviewReferenceId !== _reviewShotId
        ? normalizeShotData(_reviewReferenceFull)
        : null;
    renderShotGraph?.(
      shotReviewGraphEl,
      _reviewCurrentFull,
  ...
  function _renderReviewMeta() {
    ...
    const time = valid
      ...
      : "--:--";
    const durTxt = Number.isFinite(d.durationSec)
    ...
    const dose = Number.isFinite(d.actualDoseWeight)
      ? d.actualDoseWeight
      : null;
    ...
    const grindStr =
      grind != null ? `${_SR_GEAR_SVG} ${_escapeHtml(String(grind))}` : null;
    const doseStr = dose != null ? `${dose.toFixed(0)}g` : "—";
    const outStr =
      out != null
        ? `${est ? "~" : ""}${est ? Math.round(out) : out.toFixed(1)}${unit}`
        : "—";
    const ratioStr =
      dose != null && out != null && dose > 0
        ? ` (1:${(out / dose).toFixed(1)})`
        : "";
    const row2 = [grindStr, `${doseStr} → ${outStr}${ratioStr}`]
      .filter(Boolean)
      .join(" · ");
  ...
  async function _navigateReview(delta) {
    ...
    if (
      _isReviewDirty() &&
      !(await showConfirm(
        t("profileEditor.discardChanges"),
        t("action.discard"),
      ))
    )
      return;
  ...
  function _srTile(
    ...
    const inner =
      `<span class="bean-manager-prop-label">${_escapeHtml(label)}</span>` +
  ...
  function _renderShotReviewFields() {
    ...
    if (shotReviewRecipeGridEl) {
      const sub = (html) =>
        `<div class="shot-review-prop-subgrid">${html}</div>`;
      ...
      if (d.outValue === undefined) {
        ...
        const ratio =
          Number.isFinite(dose) && dose > 0
            ? ` (1:${(d.outValue / dose).toFixed(1)})`
            : "";
        yieldDisp = `${outStr}${ratio}`;
      }
      let html =
        _srTile("time", t("shotReview.time"), d.dispDuration, {
          editable: false,
        }) +
  ...
  function openShotReview(shotId, navList = null) {
    ...
    getShotDetailsCached(shotId)
      .then((fullShot) => {
        ...
        _reviewDraft.dispDuration = Number.isFinite(secs)
          ? `${secs.toFixed(0)} s`
          : "—";
        ...
        const annYield = Number(
          fullShot?.annotations?.actualYield ??
            fullShot?.annotations?.extras?.actualYield ??
            ann?.extras?.actualYield,
        );
        ...
        _reviewAnalysisRows =
          window.NSXUI?.renderShotAnalysis(normalizeShotData(fullShot)) || [];
        _renderShotReviewFields();
        _renderReviewMeta();
      ...
      .catch(() => {
        ...
        const estYield =
          ann.extras?.virtualScale === true
            ? (ann.extras?.actualYield ?? null)
            : null;
  ...
  document
    .getElementById("btn-shot-review-close")
    ?.addEventListener("click", closeShotReview);
  document
    .getElementById("btn-shot-review-prev")
    ?.addEventListener("click", () => _navigateReview(+1));
  document
    .getElementById("btn-shot-review-next")
    ?.addEventListener("click", () => _navigateReview(-1));
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      if (_reviewReferenceActive) {
        ...
        _reviewReferenceId = _reviewShotId;
        _reviewReferenceFull =
          _reviewCurrentFull || NSXCore.getCachedShotDetails(_reviewShotId);
        if (!_reviewReferenceFull && _reviewShotId) {
          try {
            _reviewReferenceFull = await getShotDetailsCached(_reviewShotId);
          } catch {}
        }
      }
      _updateReferenceBtn();
  ...
  document
    ...
    ?.addEventListener("click", async () => {
      ...
      const mergedWorkflow = cachedShot
        ? {
            ...(cachedShot.workflow || {}),
            context: { ...(cachedShot.workflow?.context || {}), ...ctxPatch },
          }
        : null;
      closeShotReview();
      try {
  ...
  const profilePickerPreviewEl = document.getElementById(
    "profile-picker-preview",
  );
  const profilePickerSearchEl = document.getElementById(
    "profile-picker-search",
  );
  const profilePickerSourceEl = document.getElementById(
    "profile-picker-source",
  );
  const profilePickerModeIndicatorEl = document.getElementById(
    "profile-picker-mode-indicator",
  );
  const profilePickerAddMenuEl = document.getElementById(
    "profile-picker-add-menu",
  );
  ...
  function _isUserOwnedProfile(record) {
    ...
    const src = String(record?.metadata?.source || "")
      .trim()
      .toLowerCase();
    return !src || src === "user";
  }
  ...
  function _updateProfilePickerToolbar() {
    ...
    if (toggleHiddenBtn) {
      ...
      toggleHiddenBtn.setAttribute(
        "aria-pressed",
        String(_profilePickerShowHidden),
      );
  ...
  function _matchesProfileSource(record, source) {
    ...
    if (source === "favorites")
      return _profileFavorites.has(String(record?.id || ""));
  ...
  const _ensureProfilesWithHiddenLoaded = (force = false) =>
    NSXCore.loadProfilesWithHidden(force);
  const _ensureDeletedProfilesLoaded = (force = false) =>
    NSXCore.loadDeletedProfiles(force);
  ...
  function _profileMetrics(profile) {
    ...
    if (Number(profile?.target_weight) > 0)
      stopAt = t("profileEditor.stopAtWeight").replace(
        "{weight}",
        Number(profile.target_weight).toFixed(0),
      );
    else if (Number(profile?.target_volume) > 0)
      stopAt = t("profileEditor.stopAtVolume").replace(
        "{volume}",
        Number(profile.target_volume).toFixed(0),
      );
  ...
  function _profileSparkSvg(
    ...
    const pressureValues = frames
      .map((f) => Number(f?.pressure))
      .filter(Number.isFinite);
    const flowValues = frames
      .map((f) => Number(f?.flow))
      .filter(Number.isFinite);
    ...
    const yPressure = (value) => {
      const v = Number.isFinite(value)
        ? Math.max(0, Math.min(pressureMax, value))
        : 0;
    ...
    const yTemp = (value) => {
      const v = Number.isFinite(value)
        ? Math.max(tempMin, Math.min(tempMax, value))
        : tempMin;
      return (
        plotTop +
        3 +
    ...
    for (let i = 0; i <= 3; i++) {
      ...
      const y = yPressure(v);
      gridLines.push(
        `<line x1="${plotLeft}" y1="${y.toFixed(2)}" x2="${(plotLeft + plotW).toFixed(2)}" y2="${y.toFixed(2)}" stroke="${clr.grid}" stroke-width="1"></line>`,
      );
      leftTicks.push(
        `<text x="${(plotLeft - 8).toFixed(2)}" y="${(y + 3).toFixed(2)}" text-anchor="end" fill="${clr.tickText}" font-size="${tickFontSize}">${v.toFixed(0)}</text>`,
      );
    }
    ...
    for (const v of tempTickVals) {
      const y = yTemp(v);
      rightTicks.push(
        `<text x="${(plotLeft + plotW + 8).toFixed(2)}" y="${(y + 3).toFixed(2)}" text-anchor="start" fill="${clr.tickText}" font-size="${tickFontSize}">${v.toFixed(0)}</text>`,
      );
    }
    ...
    for (let i = 0; i < xTickCount; i++) {
      ...
      xTickLabels.push(
        `<text x="${x.toFixed(2)}" y="${(xAxisY + 16).toFixed(2)}" text-anchor="middle" fill="${clr.xTickText}" font-size="${tickFontSize}">${lbl}s</text>`,
      );
    }
    ...
    for (let idx = 0; idx < stageSegments.length; idx++) {
      ...
      if (labelStripH > 0)
        stageBands.push(
          `<rect x="${seg.x0.toFixed(2)}" y="${labelStripTop}" width="${w.toFixed(2)}" height="${labelStripH}" fill="${seg.odd ? clr.stageBandOdd : clr.stageBandEven}"></rect>`,
        );
      const cx = seg.x0 + w / 2;
      ...
      if (shouldShowLabel) {
        ...
        stageLabels.push(
          `<text x="${cx.toFixed(2)}" y="${labelY}" text-anchor="middle" fill="${clr.stageLabel}" font-size="${tickFontSize}">${safeLabel}</text>`,
        );
      }
      if (!seg.isLast) {
        stageSeparators.push(
          `<line x1="${seg.x1.toFixed(2)}" y1="${labelStripTop}" x2="${seg.x1.toFixed(2)}" y2="${(plotTop + plotH).toFixed(2)}" stroke="${clr.stageSep}" stroke-width="1"></line>`,
        );
      }
    }

    const sparkFramesData = _escapeHtml(
      JSON.stringify(
        frames.map((f) => ({
    ...
    const legGap = 20;
    const leg1W =
      lineLen + lineGap + Math.ceil("Pressure (bar)".length * charW);
    ...
    const legendStartX = centerLegend
      ? Math.round(plotLeft + plotW / 2 - legendTotalW / 2)
      : 18;
  ...
  function _setupProfileSparkInteraction(containerEl) {
    ...
    let frames;
    try {
      frames = JSON.parse(svgEl.dataset.frames || "[]");
    } catch {
      return;
    }
    ...
    const lblEls = {};
    svgEl.querySelectorAll(".pspark-lbl").forEach((el) => {
      lblEls[el.dataset.key] = el;
    });
    ...
    function showAt(clientX) {
      ...
      const { f, svgX } = hit;
      if (lblEls.pressure)
        lblEls.pressure.textContent = `${f.p.toFixed(1)} bar`;
    ...
    svgEl.addEventListener("touchstart", (e) => showAt(e.touches[0].clientX), {
      passive: true,
    });
    svgEl.addEventListener("touchmove", (e) => showAt(e.touches[0].clientX), {
      passive: true,
    });
  ...
  function _findRecordById(id) {
    return (NSXCore.getProfiles() || []).find(
  ...
  function _profileLooksActive(record) {
    ...
    if (
      _editSelectedProfileId &&
      record.id &&
      String(record.id) === String(_editSelectedProfileId)
    )
      return true;
  ...
  function _renderProfilePreview(record) {
    ...
    const useBtn =
      _profilePickerContext !== "home"
        ? `<button type="button" id="btn-profile-preview-use" class="profile-preview-btn profile-preview-btn-primary">${t("profilePicker.use2")}</button>`
        : "";
    ...
    document
      .getElementById("btn-profile-preview-use")
      ?.addEventListener("click", () => {
    ...
    function _pickNextId(list, removedId) {
      const sorted = [...(list || [])].sort((a, b) =>
        String(a.profile?.title || "").localeCompare(
          String(b.profile?.title || ""),
          "en",
        ),
      );
      ...
      return String(
        remaining[Math.min(idx >= 0 ? idx : 0, remaining.length - 1)].id,
      );
    }
    ...
    function _applyNextSelection(newList, nextId) {
      _profilePickerSelectedRecord =
        (nextId
          ? (newList.find((r) => String(r.id) === nextId) ?? null)
          : null) ??
        newList[0] ??
        null;
      _renderProfilePickerList();
    ...
    document
      .getElementById("btn-profile-preview-toggle-visibility")
      ?.addEventListener("click", async (e) => {
        ...
        const srcList = currentlyHidden
          ? (Array.isArray(NSXCore.getProfilesAll())
              ? NSXCore.getProfilesAll()
              : []
            ).filter((r) => r.visibility === "hidden")
          : Array.isArray(NSXCore.getProfiles())
            ? NSXCore.getProfiles()
            : [];
        ...
        try {
          ...
          await Promise.all([
            _ensureProfilesLoaded(true),
            _ensureProfilesWithHiddenLoaded(),
          ]);
          ...
          showToast(
            currentlyHidden
              ? t("toast.presetVisible")
              : t("toast.presetHidden"),
          );
        } catch (err) {
    ...
    document
      .getElementById("btn-profile-preview-edit")
      ?.addEventListener("click", () => {
        if (profilePickerModalEl) profilePickerModalEl.hidden = true;
        openProfileEditorModal(record);
      });
    document
      .getElementById("btn-profile-preview-restore")
      ?.addEventListener("click", async (e) => {
        const profileId =
          e.target
            .closest("[data-profile-id]")
            ?.getAttribute("data-profile-id") || recordId;
        const title =
          _profilePickerSelectedRecord?.profile?.title ||
          t("profileEditor.unnamed");
        const nextId = _pickNextId(
          Array.isArray(NSXCore.getDeletedProfiles())
    ...
    document
      .getElementById("btn-profile-preview-delete")
      ?.addEventListener("click", async (e) => {
        const profileId = e.target
          .closest("[data-profile-id]")
        ...
        if (_profilePickerMode === "trash") {
          ...
          if (
            !(await showConfirm(
              t("confirm.purgeProfile").replace("{name}", title),
              t("action.purge"),
            ))
          )
            return;
  ...
  function _profileGroupOf(title) {
    ...
    if (sep > 0)
      return {
        group: title.slice(0, sep).trim(),
        name: title.slice(sep + 1).trim(),
      };
  ...
  function _renderProfileItem(record, displayTitle, grouped) {
    const selected =
      _profilePickerSelectedRecord &&
      String(_profilePickerSelectedRecord.id) === String(record.id);
    const active = _profileLooksActive(record);
    const id = _escapeHtml(record.id || "");
    const badge = _profileSourceBadge(record);
    const badgeClass =
      badge === "D"
        ? "profile-picker-badge-d"
        : badge === "V"
          ? "profile-picker-badge-v"
          : "profile-picker-badge-u";
    return `
    ...
    if (isMy && !q) {
      ...
      for (const r of ordered) {
        ...
        if (group) {
          ...
          entries.push({
            sortKey: r.profile?.title || "",
            render: () => _renderProfileItem(r, r.profile?.title || "", false),
          });
        }
      }
      for (const [group, members] of groups) {
        ...
        entries.push({
          ...
          render: () => {
            const rows = [
              `<button type="button" class="profile-picker-group-header${collapsed ? " is-collapsed" : ""}" data-group="${_escapeHtml(group)}">
          ...
          <span class="profile-picker-group-name">${_escapeHtml(group)}</span>
            ...
            if (!collapsed) {
              for (const { r, name } of members)
                rows.push(_renderProfileItem(r, name, true));
            }
            return rows.join("");
          },
        });
      }
      entries.sort((a, b) => a.sortKey.localeCompare(b.sortKey, "en"));
  ...
  function _renderProfileInfoBody(record, readOnly = false) {
    ...
    if (!profile) {
      profileInfoBodyEl.innerHTML =
        '<div class="profile-picker-placeholder">Profile not found</div>';
      return;
    }
    const frames = _extractFrames(profile);
    const metrics = _profileMetrics(profile);
    ...
    <div class="profile-steps">
      ${frames
        .map((frame, idx) => {
          const pump =
            frame?.pump === "flow"
              ? t("profileEditor.flow")
              : t("profileEditor.pressure");
          const value =
            frame?.pump === "flow"
    ...
    document
      .getElementById("btn-profile-info-delete")
      ?.addEventListener("click", async (e) => {
  ...
  async function _openProfileInfoModal(record, readOnly = false) {
    ...
    if (!resolved?.profile && resolved?.id && fetchProfileById) {
      try {
        ...
        const normalized = _normalizeProfileRecord(
          one?.profile ? one : { ...one, id: resolved.id },
        );
    ...
    if (!readOnly) {
      document
        .getElementById("btn-profile-info-edit-inline")
        ?.addEventListener("click", () => {
          if (profileInfoModalEl) profileInfoModalEl.hidden = true;
          if (profilePickerModalEl) profilePickerModalEl.hidden = true;
  ...
  const profileEditorFramesEl = document.getElementById(
    "profile-editor-frames",
  );
  const profileEditorChartEl = document.getElementById("profile-editor-chart");
  const profileEditorDirtyBadgeEl = document.getElementById(
    "profile-editor-dirty-badge",
  );
  const profileEditorStopConfigEl = document.getElementById(
    "profile-editor-stop-config",
  );
  ...
  function _peditorFramesPayload() {
    return _peditorFrames.map((frame) => {
      const payload = {
        ...
        sensor: frame.sensor,
        limiter: {
          value: frame.limiterEnabled ? frame.limiterValue : 0,
          range:
            frame.pump === "pressure"
              ? _peditorLimiterFlowRange
              : _peditorLimiterPressureRange,
        },
  ...
  function _peditorRenderStopControls() {
    const stopWeightEl = document.getElementById(
      "profile-editor-stop-weight-enabled",
    );
    const stopVolumeEl = document.getElementById(
      "profile-editor-stop-volume-enabled",
    );
    const stopWeightValEl = document.getElementById(
      "profile-editor-stop-weight-val",
    );
    const stopVolumeValEl = document.getElementById(
      "profile-editor-stop-volume-val",
    );
    const stopVolumeStartValEl = document.getElementById(
      "profile-editor-stop-volume-start-val",
    );
    const stopWeightStepperEl = document.getElementById(
      "profile-editor-stop-weight-stepper",
    );
    const stopVolumeStepperEl = document.getElementById(
      "profile-editor-stop-volume-stepper",
    );
    const stopVolumeStartStepperEl = document.getElementById(
      "profile-editor-stop-volume-start-stepper",
    );
    const maxStageIndex = Math.max(0, (_peditorFrames.length || 1) - 1);
    _peditorStopVolumeStartIndex = Math.max(
      0,
      Math.min(_peditorStopVolumeStartIndex, maxStageIndex),
    );
    ...
    if (stopWeightValEl)
      stopWeightValEl.textContent = `${_peditorStopWeightValue}g`;
    if (stopVolumeValEl)
      stopVolumeValEl.textContent = `${_peditorStopVolumeValue}ml`;
    const _svPhaseName =
      _peditorFrames[_peditorStopVolumeStartIndex]?.name?.trim() ||
      `Phase ${_peditorStopVolumeStartIndex + 1}`;
    if (stopVolumeStartValEl)
      stopVolumeStartValEl.textContent = `ab ${_svPhaseName}`;
    if (stopWeightStepperEl)
      stopWeightStepperEl.hidden = !_peditorStopWeightEnabled;
    if (stopVolumeStepperEl) stopVolumeStepperEl.hidden = false;
    if (stopVolumeStartStepperEl)
      stopVolumeStartStepperEl.hidden = !_peditorStopVolumeEnabled;

    const groupTempValEl = document.getElementById("peditor-group-temp-val");
    if (groupTempValEl)
      groupTempValEl.textContent = `${_peditorGroupTemp.toFixed(1)} °C`;

    const tankTempToggleEl = document.getElementById(
      "peditor-tank-temp-enabled",
    );
    const tankTempValEl = document.getElementById("peditor-tank-temp-val");
    const tankTempStepperEl = document.getElementById(
      "peditor-tank-temp-stepper",
    );
    ...
    const limiterFlowRangeValEl = document.getElementById(
      "peditor-limiter-flow-range-val",
    );
    const limiterPressureRangeValEl = document.getElementById(
      "peditor-limiter-pressure-range-val",
    );
    if (limiterFlowRangeValEl)
      limiterFlowRangeValEl.textContent = _peditorLimiterFlowRange.toFixed(2);
    if (limiterPressureRangeValEl)
      limiterPressureRangeValEl.textContent =
        _peditorLimiterPressureRange.toFixed(2);
  }
  ...
  function _peditorBuildProfile() {
    ...
    base.legacy_profile_type = String(
      base.legacy_profile_type || "settings_2c",
    );
    base.reference_file = String(base.reference_file ?? "");
    delete base.profile_notes;

    base.target_weight = _peditorStopWeightEnabled
      ? _peditorStopWeightValue
      : 0;
    base.target_volume = _peditorStopVolumeEnabled
      ? _peditorStopVolumeValue
      : 0;
    ...
    if (
      Array.isArray(_peditorOriginalProfile?.frames) ||
      !Array.isArray(_peditorOriginalProfile?.steps)
    ) {
  ...
  function _peditorBuildCreateFallback(profile) {
      ...
      switch (et) {
        case "pressure_under":
          return {
            type: "pressure",
            condition: "under",
            value: toNum(frame?.exit_pressure_under, 0),
          };
        case "flow_over":
          return {
            type: "flow",
            condition: "over",
            value: toNum(frame?.exit_flow_over, 0),
          };
        case "flow_under":
          return {
            type: "flow",
            condition: "under",
            value: toNum(frame?.exit_flow_under, 0),
          };
        case "weight":
          return {
            type: "weight",
            condition: "over",
            value: toNum(frame?.exit_weight, 0),
          };
        default:
          return {
            type: "pressure",
            condition: "over",
            value: toNum(frame?.exit_pressure_over, 0),
          };
      }
    }
    ...
    const steps = _extractFrames(safe).map((f, idx) => {
      ...
      if (origHasFlow || pump === "flow")
        step.flow = Math.max(0, toNum(frame.flow, 0));
      if (origHasPressure || pump === "pressure")
        step.pressure = Math.max(0, toNum(frame.pressure, 0));
    ...
    profileResult.title =
      String(safe.title || _peditorTitle || "").trim() ||
      t("profileEditor.defaultTitle");
    ...
    profileResult.target_volume_count_start = Math.max(
      0,
      Math.trunc(toNum(safe.target_volume_count_start, 0)),
    );
  ...
  function _peditorFormatVal(f) {
    return f.pump === "flow"
      ? `${f.flow.toFixed(1)} ml/s`
      : `${f.pressure.toFixed(1)} bar`;
  }
  ...
  function _peditorExitFromObject(exit) {
    ...
    if (type === "flow") {
      return {
        type: condition === "under" ? "flow_under" : "flow_over",
        value,
        enabled: true,
      };
    }
    ...
    return {
      type: condition === "under" ? "pressure_under" : "pressure_over",
      value,
      enabled: true,
    };
  }
  ...
  function _peditorRenderChart() {
    if (profileEditorChartEl) {
      profileEditorChartEl.innerHTML = _profileSparkSvg(
        _peditorBuildProfile(),
    ...
    if (phasesEl) {
      const exitTypeLabel = (et) => {
        switch (et) {
          case "pressure_under":
            return t("profileEditor.exitPressureUnder");
          case "flow_over":
            return t("profileEditor.exitFlowOver");
          case "flow_under":
            return t("profileEditor.exitFlowUnder");
          case "weight":
            return t("profileEditor.exitWeightOver");
          default:
            return t("profileEditor.exitPressureOver");
        }
      };
      phasesEl.innerHTML =
        _peditorFrames
          .map((f, i) => {
            const bullets = [];
            bullets.push(
              t("profileEditor.reviewTemp").replace(
                "{temp}",
            ...
            const transLabel =
              f.transition === "smooth"
                ? t("profileEditor.reviewSmoothTrans")
                : t("profileEditor.reviewFastTrans");
            bullets.push(
              `${modeLabel} ${_peditorFormatVal(f)} – ${transLabel}`,
            );
            if (f.seconds > 0)
              bullets.push(
                t("profileEditor.reviewMaxSecs").replace(
                  "{secs}",
                  f.seconds.toFixed(1),
                ),
              );
            if (f.exitEnabled) {
              bullets.push(
                `${t("profileEditor.reviewContinueWhen")} ${exitTypeLabel(f.exitType)} ${_peditorFormatExitVal(f)}`,
              );
            }
            const name =
              f.name || t("profileEditor.phaseN").replace("{n}", i + 1);
        ...
        <ul class="peditor-review-list">${bullets.map((b) => `<li>${b}</li>`).join("")}</ul>
          ...
          .join("") ||
        `<div class="bohnen-empty-state">${t("profileEditor.reviewNoPhases")}</div>`;
    }
    ...
    if (settingsEl) {
      ...
      if (tankTemp > 0)
        items.push(
          t("profileEditor.reviewPreheat").replace(
            "{temp}",
            `<span class="peditor-review-val">${tankTemp.toFixed(1)} °C</span>`,
          ),
        );
      if (_peditorStopVolumeEnabled) {
        items.push(
          t("profileEditor.reviewTrackVolume").replace(
            "{phase}",
            `<span class="peditor-review-val">${_peditorStopVolumeStartIndex + 1}</span>`,
          ),
        );
      }
      if (_peditorStopWeightEnabled)
        items.push(
          t("profileEditor.reviewStopWeight").replace(
            "{weight}",
            `<span class="peditor-review-val">${_peditorStopWeightValue}</span>`,
          ),
        );
      if (_peditorStopVolumeEnabled)
        items.push(
          t("profileEditor.reviewStopVolume").replace(
            "{volume}",
            `<span class="peditor-review-val">${_peditorStopVolumeValue}</span>`,
          ),
        );
  ...
  function _peditorFrameTilesHtml(f, idx) {
    ...
    const _ico = (d) =>
      `<svg class="peditor-tile-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">${d}</svg>`;
    const _icoLg = (d) =>
      `<svg class="peditor-tile-icon peditor-tile-icon--lg" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">${d}</svg>`;
    const icoTherm = _ico(
      `<path d="M14 14.76V3.5a2.5 2.5 0 0 0-5 0v11.26a4.5 4.5 0 1 0 5 0z"/>`,
    );
    const icoCoffee = _ico(
    ...
    const icoPrUp = _ico(
      `<path d="M12 5v14"/><path d="m5 12 7-7 7 7"/><path d="M3 19h18"/>`,
    );
    const icoPrDn = _ico(
      `<path d="M12 19V5"/><path d="m5 12 7 7 7-7"/><path d="M3 5h18"/>`,
    );
    const icoFlUp = _ico(
      `<path d="M12 22a7 7 0 0 0 7-7c0-2-1-3.9-3-5.5s-3.5-4-4-6.5c-.5 2.5-2 4.9-4 6.5C6 11.1 5 13 5 15a7 7 0 0 0 7 7z"/><path d="m8 10 4-4 4 4"/>`,
    );
    const icoFlDn = _ico(
    ...
    <div class="peditor-section">
      ...
      <div class="peditor-tile-grid peditor-tile-grid--nowrap">
        ...
        <div class="peditor-tile peditor-tile--full-stepper peditor-tile--exit">
          ...
          ${_icoLg(
            f.exitType === "pressure_over"
              ? `<path d="M12 5v14"/><path d="m5 12 7-7 7 7"/><path d="M3 19h18"/>`
              : f.exitType === "pressure_under"
                ? `<path d="M12 19V5"/><path d="m5 12 7 7 7-7"/><path d="M3 5h18"/>`
                : f.exitType === "flow_over"
                  ? `<path d="M12 22a7 7 0 0 0 7-7c0-2-1-3.9-3-5.5s-3.5-4-4-6.5c-.5 2.5-2 4.9-4 6.5C6 11.1 5 13 5 15a7 7 0 0 0 7 7z"/><path d="m8 10 4-4 4 4"/>`
                  : `<path d="M12 22a7 7 0 0 0 7-7c0-2-1-3.9-3-5.5s-3.5-4-4-6.5c-.5 2.5-2 4.9-4 6.5C6 11.1 5 13 5 15a7 7 0 0 0 7 7z"/><path d="m8 14 4 4 4-4"/>`,
          )}
  ...
  function _peditorRenderFrames() {
    ...
    if (countEl)
      countEl.textContent = _peditorFrames.length
        ? t("profileEditor.phasesCount").replace(
            "{count}",
            _peditorFrames.length,
          )
        : t("profileEditor.phasesLabel");
    ...
    const allFrameNames = () => {
      ...
      return uniq(
        all.flatMap((r) =>
          (r.profile?.steps ?? r.profile?.frames ?? []).map((s) => s.name),
        ),
      );
    };
    ...
    profileEditorFramesEl.classList.toggle(
      "peditor-frames--beanie",
      _peditorBeanieMode,
    );
    document
      .getElementById("peditor-panel-phasen")
      ?.classList.toggle("peditor-panel--beanie", _peditorBeanieMode);
    if (_peditorBeanieMode) {
      _peditorSelectedFrameIdx = Math.max(
        0,
        Math.min(_peditorSelectedFrameIdx, _peditorFrames.length - 1),
      );
        ...
        <div class="peditor-bl-right">
          ...
          <div class="peditor-bl-detail-body">
            ${_peditorFrameTilesHtml(selFrame, selIdx)}
              ...
              : `<div class="bohnen-empty-state">${t("profileEditor.noPhases")}</div>`
          }
        </div>
      </div>`;

      profileEditorFramesEl
        .querySelectorAll(".profile-editor-frame-name")
        .forEach((el) => {
          el.addEventListener("pointerdown", (e) => {
            e.preventDefault();
            openFieldPicker(el, allFrameNames());
          });
        });

      profileEditorFramesEl
        .querySelectorAll("[data-bl-select]")
        .forEach((el) => {
      ...
      profileEditorFramesEl
        .querySelectorAll("[data-bl-action]")
        .forEach((btn) => {
          btn.addEventListener("click", () => {
            ...
            if (action === "add") {
              ...
              _peditorFrames.splice(si + 1, 0, {
                ..._peditorClone(sf),
                name: `${sf.name || `Phase ${si + 1}`} (Kopie)`,
              });
              _peditorSelectedFrameIdx = si + 1;
            } else if (action === "move-up" && sf && si > 0) {
              [_peditorFrames[si - 1], _peditorFrames[si]] = [
                _peditorFrames[si],
                _peditorFrames[si - 1],
              ];
              _peditorSelectedFrameIdx = si - 1;
              ...
              _peditorSelectedFrameIdx = Math.min(
                si,
                Math.max(0, _peditorFrames.length - 1),
              );
            }
            _peditorRenderStopControls();
            _peditorRenderFrames();
    ...
    profileEditorFramesEl.innerHTML = _peditorFrames
      .map(
        (f, idx) => `
  ...
  function _peditorFrameFrom(raw) {
    const parsedExit =
      raw?.exit && typeof raw.exit === "object"
        ? _peditorExitFromObject(raw.exit)
        : null;
    const {
      name,
      pump,
      ...
      volume,
      weight,
      // Profile-level fields: don't pack into Step
      tank_temperature,
      target_volume,
    ...
    return {
      ...
      limiterEnabled:
        limiter && typeof limiter === "object"
          ? Number(limiter.value) > 0
          : false,
      limiterValue:
        limiter && typeof limiter === "object" && Number(limiter.value) > 0
          ? Number(limiter.value)
          : 0,
      limiterRange:
        limiter && typeof limiter === "object" && Number(limiter.range) > 0
          ? Number(limiter.range)
          : 0.6,
      ...
      exitType: parsedExit
        ? parsedExit.type
        : String(exit_type || "pressure_over"),
      exitValue: parsedExit
        ? parsedExit.value
        : _peditorFrameExitValue({
            exit_type,
            exit_pressure_over,
            exit_pressure_under,
            exit_flow_over,
  ...
  function openProfileEditorModal(record) {
    ...
    _peditorStopWeightValue = _peditorStopWeightEnabled
      ? Number(profile.target_weight)
      : 36;
    _peditorStopVolumeEnabled = Number(profile.target_volume) > 0;
    _peditorStopVolumeValue = _peditorStopVolumeEnabled
      ? Number(profile.target_volume)
      : 0;
    _peditorStopVolumeStartIndex = Math.max(
      0,
      Math.trunc(Number(profile.target_volume_count_start) || 0),
    );
    ...
    _peditorTankTempValue = _peditorTankTempEnabled
      ? Number(profile.tank_temperature)
      : 0;
    _peditorLimiterFlowRange =
      Number(profile.limiter_flow_range) > 0
        ? Number(profile.limiter_flow_range)
        : 0.6;
    _peditorLimiterPressureRange =
      Number(profile.limiter_pressure_range) > 0
        ? Number(profile.limiter_pressure_range)
        : 0.6;
    ...
    if (notesEl) {
      notesEl.value = _peditorNotes;
      _autoResizeNotes(notesEl);
    }
  ...
  function _peditorAttemptClose(force = false) {
    ...
    if (
      !force &&
      _peditorRefreshDirtyState() &&
      !window.confirm(t("profileEditor.discardChanges"))
    ) {
      return false;
    }
  ...
  async function _peditorSave() {
    _peditorTitle = (
      document.getElementById("profile-editor-title")?.value || ""
    ).trim();
    _peditorAuthor = (
      document.getElementById("profile-editor-author")?.value || ""
    ).trim();
    _peditorNotes = (
      document.getElementById("profile-editor-notes")?.value || ""
    ).trim();

    if (!_peditorTitle) {
      showToast(t("toast.profileNameRequired"));
      return;
    }
    if (!_peditorFrames.length) {
      showToast(t("toast.addPhase"));
      return;
    }
    ...
    try {
      ...
      if (hasId && !isDefault) {
        if (_peditorHasExecutionChanges()) {
          ...
          try {
            await deleteProfile(_peditorRecord.id);
          } catch {
      ...
      _peditorRecord = _normalizeProfileRecord(saved) || {
        id: saved?.id ?? _peditorRecord?.id ?? null,
        profile: payload,
      };
      _peditorOriginalProfile =
        _peditorClone(_peditorRecord.profile) || _peditorClone(payload) || {};
      _peditorOriginalSnapshot = _peditorSnapshot();
      NSXCore.invalidateProfiles();
      ...
      if (isDefault) {
        ...
        if (originalId) {
          try {
            await setProfileVisibility(originalId, "hidden");
          } catch {
            /* best-effort */
          }
        }
  ...
  document
    .getElementById("btn-profile-editor-save")
    ?.addEventListener("click", _peditorSave);
  document
    .getElementById("btn-profile-editor-cancel")
    ?.addEventListener("click", () => {
  ...
  profileEditorModalEl?.addEventListener("click", (e) => {
    ...
    if (tabBtn) {
      _peditorSwitchTab(tabBtn.dataset.peditorTab);
      return;
    }
  ...
  document
    .getElementById("profile-editor-title")
    ?.addEventListener("input", (e) => {
      _peditorTitle = e.target.value;
      _peditorRefreshDirtyState();
    });
  document
    .getElementById("profile-editor-author")
    ?.addEventListener("input", (e) => {
  ...
  document
    .getElementById("profile-editor-notes")
    ?.addEventListener("input", (e) => {
  ...
  document
    .getElementById("profile-editor-stop-weight-enabled")
    ?.addEventListener("change", (e) => {
  ...
  document
    .getElementById("profile-editor-stop-volume-enabled")
    ?.addEventListener("change", (e) => {
  ...
  document
    .getElementById("btn-peditor-stop-weight-down")
    ?.addEventListener("click", () => {
      _peditorStopWeightValue = Math.max(
        1,
        _peditorStopWeightValue - _peditorWeightStepSize(false),
      );
      _peditorRenderStopControls();
      _peditorRefreshDirtyState();
    });
  document
    .getElementById("btn-peditor-stop-weight-up")
    ?.addEventListener("click", () => {
      _peditorStopWeightValue = Math.min(
        500,
        _peditorStopWeightValue + _peditorWeightStepSize(false),
      );
      _peditorRenderStopControls();
  ...
  document
    .getElementById("btn-peditor-stop-weight-down-big")
    ?.addEventListener("click", () => {
      _peditorStopWeightValue = Math.max(
        1,
        _peditorStopWeightValue - _peditorWeightStepSize(true),
      );
      _peditorRenderStopControls();
      _peditorRefreshDirtyState();
    });
  document
    .getElementById("btn-peditor-stop-weight-up-big")
    ?.addEventListener("click", () => {
      _peditorStopWeightValue = Math.min(
        500,
        _peditorStopWeightValue + _peditorWeightStepSize(true),
  ...
  document
    .getElementById("btn-peditor-stop-volume-down")
    ?.addEventListener("click", () => {
      _peditorStopVolumeValue = Math.max(
        1,
        _peditorStopVolumeValue - _peditorVolumeStepSize(false),
      );
      _peditorRenderStopControls();
      _peditorRefreshDirtyState();
    });
  document
    .getElementById("btn-peditor-stop-volume-up")
    ?.addEventListener("click", () => {
      _peditorStopVolumeValue = Math.min(
        500,
        _peditorStopVolumeValue + _peditorVolumeStepSize(false),
      );
      _peditorRenderStopControls();
  ...
  document
    .getElementById("btn-peditor-stop-volume-down-big")
    ?.addEventListener("click", () => {
      _peditorStopVolumeValue = Math.max(
        1,
        _peditorStopVolumeValue - _peditorVolumeStepSize(true),
      );
      _peditorRenderStopControls();
      _peditorRefreshDirtyState();
    });
  document
    .getElementById("btn-peditor-stop-volume-up-big")
    ?.addEventListener("click", () => {
      _peditorStopVolumeValue = Math.min(
        500,
        _peditorStopVolumeValue + _peditorVolumeStepSize(true),
  ...
  document
    .getElementById("btn-peditor-stop-volume-start-down")
    ?.addEventListener("click", () => {
      _peditorStopVolumeStartIndex = Math.max(
        0,
        _peditorStopVolumeStartIndex - _peditorPhaseIndexStepSize(false),
      );
      _peditorRenderStopControls();
      _peditorRefreshDirtyState();
    });
  document
    .getElementById("btn-peditor-stop-volume-start-up")
    ?.addEventListener("click", () => {
      const maxStageIndex = Math.max(0, (_peditorFrames.length || 1) - 1);
      _peditorStopVolumeStartIndex = Math.min(
        maxStageIndex,
        _peditorStopVolumeStartIndex + _peditorPhaseIndexStepSize(false),
      );
      _peditorRenderStopControls();
  ...
  document
    .getElementById("peditor-group-temp-val")
    ?.addEventListener("click", () => {
  ...
  document
    .getElementById("peditor-tank-temp-val")
    ?.addEventListener("click", () => {
  ...
  document
    .getElementById("profile-editor-stop-weight-val")
    ?.addEventListener("click", () => {
      openNumberPicker(
        _npMakeRange(1, 500, 1),
        _peditorStopWeightValue,
        (v) => {
  ...
  document
    .getElementById("profile-editor-stop-volume-val")
    ?.addEventListener("click", () => {
      openNumberPicker(
        _npMakeRange(1, 500, 1),
        _peditorStopVolumeValue,
  ...
  document
    .getElementById("peditor-limiter-flow-range-val")
    ?.addEventListener("click", () => {
      openNumberPicker(
        _npMakeRange(0.1, 5, 0.1),
        _peditorLimiterFlowRange,
        (v) => {
  ...
  document
    .getElementById("peditor-limiter-pressure-range-val")
    ?.addEventListener("click", () => {
      openNumberPicker(
        _npMakeRange(0.1, 5, 0.1),
        _peditorLimiterPressureRange,
  ...
  document
    .getElementById("btn-peditor-group-temp-down-big")
    ?.addEventListener("click", () => {
      const delta = -_peditorTemperatureStepSize(true);
      _peditorGroupTemp = Math.max(
        70,
        Math.round((_peditorGroupTemp + delta) * 10) / 10,
      );
      _peditorApplyGroupTempDelta(delta);
      _peditorRenderStopControls();
      _peditorRenderFrames();
  ...
  document
    .getElementById("btn-peditor-group-temp-down")
    ?.addEventListener("click", () => {
  ...
  document
    .getElementById("btn-peditor-group-temp-up")
    ?.addEventListener("click", () => {
      const delta = _peditorTemperatureStepSize(false);
      _peditorGroupTemp = Math.min(
        105,
        Math.round((_peditorGroupTemp + delta) * 10) / 10,
      );
      _peditorApplyGroupTempDelta(delta);
  ...
  document
    .getElementById("btn-peditor-group-temp-up-big")
    ?.addEventListener("click", () => {
      const delta = _peditorTemperatureStepSize(true);
      _peditorGroupTemp = Math.min(
        105,
  ...
  document
    .getElementById("peditor-tank-temp-enabled")
    ?.addEventListener("change", (e) => {
  ...
  document
    .getElementById("btn-peditor-tank-temp-down")
    ?.addEventListener("click", () => {
  ...
  document
    .getElementById("btn-peditor-tank-temp-up")
    ?.addEventListener("click", () => {
  ...
  document
    .getElementById("btn-peditor-limiter-flow-range-down")
    ?.addEventListener("click", () => {
      _peditorLimiterFlowRange = Math.max(
        0.1,
        Math.round((_peditorLimiterFlowRange - 0.1) * 100) / 100,
      );
      _peditorRenderStopControls();
      _peditorRefreshDirtyState();
    });
  document
    .getElementById("btn-peditor-limiter-flow-range-up")
    ?.addEventListener("click", () => {
      _peditorLimiterFlowRange = Math.min(
        5,
        Math.round((_peditorLimiterFlowRange + 0.1) * 100) / 100,
      );
      _peditorRenderStopControls();
  ...
  document
    .getElementById("btn-peditor-limiter-pressure-range-down")
    ?.addEventListener("click", () => {
      _peditorLimiterPressureRange = Math.max(
        0.1,
        Math.round((_peditorLimiterPressureRange - 0.1) * 100) / 100,
      );
      _peditorRenderStopControls();
      _peditorRefreshDirtyState();
    });
  document
    .getElementById("btn-peditor-limiter-pressure-range-up")
    ?.addEventListener("click", () => {
      _peditorLimiterPressureRange = Math.min(
        5,
        Math.round((_peditorLimiterPressureRange + 0.1) * 100) / 100,
      );
      _peditorRenderStopControls();
  ...
  document
    .getElementById("btn-profile-editor-add-frame")
    ?.addEventListener("click", () => {
      const last = _peditorFrames[_peditorFrames.length - 1];
      _peditorFrames.push({
        name: `Phase ${_peditorFrames.length + 1}`,
  ...
  profileEditorFramesEl?.addEventListener("input", (e) => {
    ...
    if (_peditorFrames[idx]) {
      ...
      if (_peditorBeanieMode) {
        const nameEl = profileEditorFramesEl.querySelector(
          `[data-bl-select="${idx}"] .peditor-bl-item-name`,
        );
  ...
  profileEditorFramesEl?.addEventListener("click", (e) => {
    ...
    const display = (type, text) => {
      const el = profileEditorFramesEl.querySelector(
        `[data-frame-idx="${idx}"][data-frame-display="${type}"]`,
      );
      if (el) el.textContent = text;
    };
    ...
    switch (action) {
      ...
      case "move-up":
        if (!f || idx <= 0) return;
        [_peditorFrames[idx - 1], _peditorFrames[idx]] = [
          _peditorFrames[idx],
          _peditorFrames[idx - 1],
        ];
        if (_peditorBeanieMode && _peditorSelectedFrameIdx === idx)
          _peditorSelectedFrameIdx--;
        _peditorRenderFrames();
        _peditorRenderChart();
        _peditorRefreshDirtyState();
      ...
      case "limiter-tile-toggle":
        ...
        if (f.limiterEnabled && f.limiterValue === 0)
          f.limiterValue = f.pump === "pressure" ? 8.0 : 6.0;
        _peditorRenderFrames();
        _peditorRefreshDirtyState();
        break;
      case "exit-tile": {
      ...
      case "limiter-down":
        ...
        f.limiterValue = Math.max(
          0.1,
          Math.round(
            (f.limiterValue - (action === "limiter-down-big" ? 1.0 : 0.1)) * 10,
          ) / 10,
        );
        display(
          "limiter",
      ...
      case "limiter-up":
        if (!f || !f.limiterEnabled) return;
        f.limiterValue = Math.min(
          f.pump === "pressure" ? 15 : 12,
          Math.round(
            (f.limiterValue + (action === "limiter-up-big" ? 1.0 : 0.1)) * 10,
          ) / 10,
        );
        display(
          "limiter",
      ...
      case "volume-toggle":
        if (!f) return;
        f.volumeEnabled = !f.volumeEnabled;
        if (f.volumeEnabled && (!f.volumeValue || f.volumeValue === 0))
          f.volumeValue = 36;
        _peditorRenderFrames();
        _peditorRefreshDirtyState();
        break;
      ...
      case "volume-down":
        ...
        f.volumeValue = Math.max(
          1,
          f.volumeValue - (action === "volume-down-big" ? 10 : 1),
        );
      ...
      case "volume-up":
        if (!f || !f.volumeEnabled) return;
        f.volumeValue = Math.min(
          500,
          f.volumeValue + (action === "volume-up-big" ? 10 : 1),
        );
        display("volume", f.volumeValue.toFixed(0) + " ml");
        _peditorRefreshDirtyState();
        break;
      case "weight-toggle":
        if (!f) return;
        f.weightEnabled = !f.weightEnabled;
        if (f.weightEnabled && (!f.weightValue || f.weightValue === 0))
          f.weightValue = 36;
        _peditorRenderFrames();
        _peditorRefreshDirtyState();
        break;
      ...
      case "weight-down":
        ...
        f.weightValue = Math.max(
          0.1,
          Math.round(
            (f.weightValue - (action === "weight-down-big" ? 5 : 0.5)) * 10,
          ) / 10,
        );
      ...
      case "weight-up":
        if (!f || !f.weightEnabled) return;
        f.weightValue = Math.min(
          500,
          Math.round(
            (f.weightValue + (action === "weight-up-big" ? 5 : 0.5)) * 10,
          ) / 10,
        );
        display("weight", f.weightValue.toFixed(1) + " g");
        _peditorRefreshDirtyState();
      ...
      case "val-down-big":
        ...
        if (f.pump === "flow")
          f.flow = Math.max(
            0,
            Math.round(
              (f.flow - _peditorTargetStepSize(f, action === "val-down-big")) *
                10,
            ) / 10,
          );
        else
          f.pressure = Math.max(
            ...
            Math.round(
              (f.pressure -
                _peditorTargetStepSize(f, action === "val-down-big")) *
                10,
            ) / 10,
          );
      ...
      case "val-up-big":
        ...
        if (f.pump === "flow")
          f.flow = Math.min(
            15,
            Math.round(
              (f.flow + _peditorTargetStepSize(f, action === "val-up-big")) *
                10,
            ) / 10,
          );
        else
          f.pressure = Math.min(
            ...
            Math.round(
              (f.pressure +
                _peditorTargetStepSize(f, action === "val-up-big")) *
                10,
            ) / 10,
          );
        display("val", _peditorFormatVal(f));
      ...
      case "temp-down-big":
        if (!f) return;
        f.temperature = Math.max(
          70,
          Math.round(
            (f.temperature -
              _peditorTemperatureStepSize(action === "temp-down-big")) *
              10,
          ) / 10,
        );
      ...
      case "temp-up-big":
        if (!f) return;
        f.temperature = Math.min(
          105,
          Math.round(
            (f.temperature +
              _peditorTemperatureStepSize(action === "temp-up-big")) *
              10,
          ) / 10,
        );
        display("temp", `${f.temperature.toFixed(1)}°C`);
      ...
      case "dur-down-big":
        ...
        f.seconds = Math.max(
          0,
          Math.round(
            (f.seconds - _peditorDurationStepSize(action === "dur-down-big")) *
              10,
          ) / 10,
        );
      ...
      case "dur-up-big":
        ...
        f.seconds = Math.min(
          120,
          Math.round(
            (f.seconds + _peditorDurationStepSize(action === "dur-up-big")) *
              10,
          ) / 10,
        );
        display("dur", `${f.seconds.toFixed(1)} s`);
      ...
      case "exit-down-big": {
        ...
        f.exitValue = Math.max(
          _peditorExitMin(f),
          Math.round((f.exitValue - step) * 10) / 10,
        );
  ...
  profileEditorFramesEl?.addEventListener("click", (e) => {
    ...
    switch (span.dataset.frameDisplay) {
      ...
      case "exit": {
        ...
        openNumberPicker(
          _npMakeRange(_peditorExitMin(f), _peditorExitMax(f), step),
          f.exitValue,
          (v) => {
            f.exitValue = v;
      ...
      if (context === "home" || context === "recipe") {
        ...
        if (cancelBtn)
          cancelBtn.textContent =
            context === "home" ? t("action.close") : t("action.cancel");
      } else {
        if (useBtn) useBtn.hidden = false;
        if (cancelBtn) cancelBtn.textContent = t("action.cancel");
      }
      if (isRecipe) {
        const records = Array.isArray(NSXCore.getProfiles())
          ? NSXCore.getProfiles()
          : [];
        ...
        if (!match) {
          const title =
            _editSelectedProfileObj?.title ||
            document.getElementById("edit-profile")?.value?.trim();
          if (title && title !== "—")
            match = records.find((r) => r.profile?.title === title);
        }
        if (match) {
          ...
          requestAnimationFrame(() => {
            profilePickerListEl
              ?.querySelector(".profile-picker-item.is-selected")
  ...
  function _syncEditRatio() {
    ...
    if (el)
      el.textContent =
        _editDose > 0 ? `1:${(_editYield / _editDose).toFixed(1)}` : "—";
  }
  function _setEditGrind(v) {
    ...
    if (el)
      el.textContent =
        _editGrind % 1 === 0
          ? String(_editGrind)
          : _editGrind.toFixed(2).replace(/0+$/, "").replace(/\.$/, "");
  }
  ...
  function openWorkflowEditModal(index) {
    ...
    _editPickedRoaster =
      workflow.coffeeRoaster !== "—" ? workflow.coffeeRoaster : "";
    _editPickedBeanName =
      workflow.coffeeName && workflow.coffeeName !== "—"
        ? workflow.coffeeName
        : "";
    ...
    _editPickedGrinderModel =
      workflow.grinderModel !== "—" ? workflow.grinderModel : "";
    ...
    if (
      _editPickedGrinderId &&
      !NSXCore.getGrinders().find((g) => g.id === _editPickedGrinderId)
    ) {
      NSXCore.loadGrinders()
        .then(() => updateGrinderPickerDisplay())
        .catch(() => {});
    }
    document.getElementById("edit-profile").value =
      workflow.profileTitle !== "—" ? workflow.profileTitle : "";
    ...
    const gwProfile =
      gwf?.profile && typeof gwf.profile === "object" ? gwf.profile : null;
    _editSelectedProfileObj = gwProfile
      ? JSON.parse(JSON.stringify(gwProfile))
      : null;
    ...
    _setEditGroupTemp(
      Number(workflow.groupTemp || gwf?.profile?.groupTemp) || 93,
    );
    ...
    document
      .getElementById("edit-card-bean")
      ?.classList.remove("recipe-edit-card--identity-changed");
    document
      .getElementById("edit-card-grinder")
      ?.classList.remove("recipe-edit-card--identity-changed");
    document
      .getElementById("edit-card-machine")
      ?.classList.remove("recipe-edit-card--identity-changed");
  ...
  function openWorkflowCreateModal() {
    ...
    if (_cachedGrinders.length > 0) {
      ...
      updateGrinderPickerDisplay();
      NSXCore.loadGrinders()
        .then(() => _applyAutoGrinder(NSXCore.getGrinders()))
        .catch(() => {});
    }
    document.getElementById("edit-profile").value = "";
    _syncProfileDisplay();
    ...
    const saveBtn = document.getElementById("btn-edit-save");
    if (saveBtn) {
      saveBtn.textContent = t("action.create");
      saveBtn.classList.remove("modal-btn-save--fork");
    }
  ...
  document
    ...
    ?.addEventListener("input", _updateEditDirtyState);
  ...
  document
    .getElementById("edit-profile")
    ?.addEventListener("click", () => openProfilePickerModal("recipe"));
  document
    .getElementById("btn-edit-pick-profile")
    ?.addEventListener("click", () => openProfilePickerModal("recipe"));
  ...
  document
    .getElementById("btn-edit-profile-info")
    ?.addEventListener("click", async () => {
      if (_editSelectedProfileObj) {
        _openProfileInfoModal(
          { id: _editSelectedProfileId, profile: _editSelectedProfileObj },
          true,
        );
        return;
      }
      ...
      try {
        ...
        const match = records.find(
          (r) => String(r.profile?.title || "").trim() === title,
        );
        if (!match) {
  ...
  document
    .getElementById("btn-profile-picker-cancel")
    ?.addEventListener("click", () => {
      if (profilePickerModalEl) profilePickerModalEl.hidden = true;
      if (profilePickerAddMenuEl) profilePickerAddMenuEl.hidden = true;
      const useBtn = document.getElementById("btn-profile-picker-use");
  ...
  document
    .getElementById("btn-profile-picker-use")
    ?.addEventListener("click", () => {
  ...
  document
    .getElementById("btn-profile-picker-open-add")
    ?.addEventListener("click", () => {
      if (profilePickerAddMenuEl)
        profilePickerAddMenuEl.hidden = !profilePickerAddMenuEl.hidden;
    });

  document
    .getElementById("btn-profile-picker-open-trash")
    ?.addEventListener("click", () => {
      if (profilePickerAddMenuEl) profilePickerAddMenuEl.hidden = true;
  ...
  document
    .getElementById("btn-profile-picker-toggle-hidden")
    ?.addEventListener("click", () => {
      _profilePickerShowHidden = !_profilePickerShowHidden;
      if (_profilePickerShowHidden) {
        _ensureProfilesWithHiddenLoaded().then(() =>
          _renderProfilePickerList(),
        );
      }
      _updateProfilePickerToolbar();
  ...
  document
    .getElementById("btn-profile-picker-empty-trash")
    ?.addEventListener("click", async () => {
      ...
      if (!confirm(t("confirm.emptyTrash").replace("{count}", records.length)))
        return;
      let failed = 0;
  ...
  document
    .getElementById("btn-profile-picker-new-empty")
    ?.addEventListener("click", () => {
      if (profilePickerAddMenuEl) profilePickerAddMenuEl.hidden = true;
  ...
  document
    .getElementById("btn-profile-picker-new-preset")
    ?.addEventListener("click", () => {
      if (profilePickerAddMenuEl) profilePickerAddMenuEl.hidden = true;
  ...
  document
    .getElementById("btn-profile-picker-import-file")
    ?.addEventListener("click", () => {
      if (profilePickerAddMenuEl) profilePickerAddMenuEl.hidden = true;
  ...
  document
    .getElementById("btn-profile-picker-import-visualizer")
    ?.addEventListener("click", () => {
      if (profilePickerAddMenuEl) profilePickerAddMenuEl.hidden = true;
  ...
  document
    ...
    ?.addEventListener("change", async (e) => {
      ...
      try {
        ...
        if (
          !Array.isArray(profileData.steps) &&
          !Array.isArray(profileData.frames)
        ) {
          throw new Error("No steps found");
        }

        const title =
          String(profileData.title || "").trim() ||
  ...
  async function _importFromVisualizer(shareCode) {
    ...
    const sharedRes = await fetch(
      `https://visualizer.coffee/api/shots/shared?code=${encodeURIComponent(code)}`,
    );
    if (sharedRes.status === 404)
      throw new Error(t("toast.visualizerNotFound"));
    ...
    const profileRes = await fetch(
      `https://visualizer.coffee/api/shots/${encodeURIComponent(shotId)}/profile?format=json`,
    );
    if (!profileRes.ok)
      throw new Error(`Visualizer profile ${profileRes.status}`);
    const json = await profileRes.json();
    const profileData =
      json?.profile && typeof json.profile === "object" ? json.profile : json;
    if (!profileData || typeof profileData !== "object")
  ...
  document
    .getElementById("btn-profile-picker-back-my")
    ?.addEventListener("click", () => {
      _setProfilePickerMode("my");
    });

  document
    .getElementById("btn-profile-info-back")
    ?.addEventListener("click", () => {
      if (profileInfoModalEl) profileInfoModalEl.hidden = true;
    });

  document
    .getElementById("btn-profile-info-use")
    ?.addEventListener("click", () => {
      if (!_profilePickerSelectedRecord) {
  ...
  document
    .getElementById("btn-edit-dose-down")
    ?.addEventListener("click", () => _setEditDose(_editDose - 0.1));
  document
    .getElementById("btn-edit-dose-up")
    ?.addEventListener("click", () => _setEditDose(_editDose + 0.1));
  document
    .getElementById("btn-edit-dose-down-big")
    ?.addEventListener("click", () => _setEditDose(_editDose - 1));
  document
    .getElementById("btn-edit-dose-up-big")
    ?.addEventListener("click", () => _setEditDose(_editDose + 1));
  document
    .getElementById("btn-edit-yield-down")
    ?.addEventListener("click", () => _setEditYield(_editYield - 1));
  document
    .getElementById("btn-edit-yield-up")
    ?.addEventListener("click", () => _setEditYield(_editYield + 1));
  document
    .getElementById("btn-edit-yield-down-big")
    ?.addEventListener("click", () => _setEditYield(_editYield - 10));
  document
    .getElementById("btn-edit-yield-up-big")
    ?.addEventListener("click", () => _setEditYield(_editYield + 10));
  document
    .getElementById("btn-edit-grind-down")
    ?.addEventListener("click", () =>
      _setEditGrind(_editGrind - _getGrinderSteps().small),
    );
  document
    .getElementById("btn-edit-grind-up")
    ?.addEventListener("click", () =>
      _setEditGrind(_editGrind + _getGrinderSteps().small),
    );
  document
    .getElementById("btn-edit-grind-down-big")
    ?.addEventListener("click", () =>
      _setEditGrind(_editGrind - _getGrinderSteps().big),
    );
  document
    .getElementById("btn-edit-grind-up-big")
    ?.addEventListener("click", () =>
      _setEditGrind(_editGrind + _getGrinderSteps().big),
    );
  document
    .getElementById("btn-edit-steam-temp-down")
    ?.addEventListener("click", () => _setEditSteamTemp(_editSteamTemp - 5));
  document
    .getElementById("btn-edit-steam-temp-up")
    ?.addEventListener("click", () => _setEditSteamTemp(_editSteamTemp + 5));
  document
    .getElementById("btn-edit-steam-dur-down")
    ?.addEventListener("click", () => _setEditSteamDur(_editSteamDur - 5));
  document
    .getElementById("btn-edit-steam-dur-up")
    ?.addEventListener("click", () => _setEditSteamDur(_editSteamDur + 5));
  document
    .getElementById("btn-edit-hw-temp-down")
    ?.addEventListener("click", () => _setEditHwTemp(_editHwTemp - 5));
  document
    .getElementById("btn-edit-hw-temp-up")
    ?.addEventListener("click", () => _setEditHwTemp(_editHwTemp + 5));
  document
    .getElementById("btn-edit-hw-vol-down")
    ?.addEventListener("click", () => _setEditHwVol(_editHwVol - 10));
  document
    .getElementById("btn-edit-hw-vol-up")
    ?.addEventListener("click", () => _setEditHwVol(_editHwVol + 10));
  document
    .getElementById("btn-edit-group-temp-down")
    ?.addEventListener("click", () => _setEditGroupTemp(_editGroupTemp - 1));
  document
    .getElementById("btn-edit-group-temp-up")
    ?.addEventListener("click", () => _setEditGroupTemp(_editGroupTemp + 1));
  document
    .getElementById("btn-edit-group-temp-down-big")
    ?.addEventListener("click", () => _setEditGroupTemp(_editGroupTemp - 10));
  document
    .getElementById("btn-edit-group-temp-up-big")
    ?.addEventListener("click", () => _setEditGroupTemp(_editGroupTemp + 10));

  /* ── Tap-to-edit for Recipe Edit display values ─────── */
  document
    .getElementById("edit-dose-display")
    ?.addEventListener("click", () =>
      openNumberPicker(
        _npMakeRange(0, 30, 0.1),
        _editDose,
        (v) => _setEditDose(v),
        1,
      ),
  ...
  document
    .getElementById("edit-yield-display")
    ?.addEventListener("click", () =>
      openNumberPicker(_npMakeRange(0, 200, 1), _editYield, (v) =>
        _setEditYield(v),
      ),
    );
  document
    .getElementById("edit-group-temp-display")
    ?.addEventListener("click", () =>
      openNumberPicker(_npMakeRange(80, 100, 1), _editGroupTemp, (v) =>
        _setEditGroupTemp(v),
      ),
    );
  document
    .getElementById("edit-grind-display")
    ?.addEventListener("click", () => {
      const step = _getGrinderSteps().small;
      const dp = step < 1 ? Math.ceil(-Math.log10(step)) : 0;
      openNumberPicker(
        _npMakeRange(0, 200, step),
        _editGrind,
        (v) => _setEditGrind(v),
        dp,
      );
  ...
  document
    .getElementById("btn-edit-save")
    ?.addEventListener("click", async () => {
      if (!workflowEditModalEl) return;
      const index = workflowEditModalEl._editIndex;
      const isCreate = index === -1;
      if (!isCreate && (!Number.isInteger(index) || !workflowItems[index]))
        return;
      ...
      const setting =
        _editGrind % 1 === 0 ? String(_editGrind) : _editGrind.toFixed(1);
      ...
      const updated = {
        ...
        selectedProfileId:
          _editSelectedProfileId ?? original.selectedProfileId ?? null,
        ratio,
      ...
      if (isCreate) {
        ...
        selectedWorkflowIndex = 0;
        _autoAssignPresetSlot(updated.id);
      } else {
  ...
  function _renderVolumeStopStatus(calibration) {
    ...
    if (!samples || samples.length === 0) {
      ...
      el.className =
        "recipe-edit-calib-status recipe-edit-calib-status--uncalibrated";
    } else if (samples.length < 3) {
      el.textContent = t("recipeEdit.calibWarning");
      el.className =
        "recipe-edit-calib-status recipe-edit-calib-status--warning";
    } else {
  ...
  function _renderTagSuggestions() {
    ...
    const allTags = [
      ...new Set(
        workflowItems.flatMap((w) => (Array.isArray(w.tags) ? w.tags : [])),
      ),
    ];
  ...
  document
    .getElementById("edit-volume-stop-toggle")
    ?.addEventListener("change", (e) => {
  ...
  document
    .getElementById("btn-virtual-scale-info")
    ?.addEventListener("click", () => {
  ...
  document
    .getElementById("btn-virtual-scale-reset")
    ?.addEventListener("click", () => {
  ...
  document
    .getElementById("btn-virtual-scale-info-close")
    ?.addEventListener("click", () => {
      document.getElementById("virtual-scale-info-modal").hidden = true;
    });
  document
    .getElementById("virtual-scale-info-modal")
    ?.addEventListener("click", (e) => {
  ...
  async function _resolveEditBeanDetails() {
    ...
    let bean = NSXCore.getBeans().find(
      (b) => b.roaster === _editPickedRoaster && b.name === _editPickedBeanName,
    );
    if (!bean) {
      try {
        await NSXCore.loadBeans(true);
        bean = NSXCore.getBeans().find(
          (b) =>
    ...
    if (bean) {
      ...
      _editPickedBeanVariety = Array.isArray(bean.variety)
        ? bean.variety.join(", ")
        : bean.variety || "";
  ...
  async function _refreshEditBeanAgeMessage() {
    ...
    if (_editPickedBatchRoastDate) {
      _setEditBeanAgeMessage(
        t("beanEditor.age").replace(
          "{age}",
          formatBatchAge(_editPickedBatchRoastDate),
        ),
      );
      return;
    }
    ...
    if (roastDate) {
      ...
      _setEditBeanAgeMessage(
        t("beanEditor.age").replace("{age}", formatBatchAge(roastDate)),
      );
    } else {
  ...
  function updateGrinderPickerDisplay() {
    ...
    if (burrsizeEl)
      burrsizeEl.textContent = g?.burrSize ? `${g.burrSize} mm` : "—";
    _updateEditDirtyState();
  }
  ...
  function _updateEditDirtyState() {
    ...
    const beanChanged =
      roaster !== _originalIdentity.roaster || bean !== _originalIdentity.bean;
    ...
    document
      .getElementById("edit-card-bean")
      ?.classList.toggle("recipe-edit-card--identity-changed", beanChanged);
    document
      .getElementById("edit-card-grinder")
      ?.classList.toggle("recipe-edit-card--identity-changed", grinderChanged);
    document
      .getElementById("edit-card-machine")
      ?.classList.toggle("recipe-edit-card--identity-changed", profileChanged);
  ...
  document
    .getElementById("btn-edit-pick-bean")
    ?.addEventListener("click", () => {
  ...
  function _renderGrinderPickerTiles(grinders) {
    ...
    for (const g of grinders) {
      ...
      const burrTypeLabel =
        g.burrType === "conical"
          ? t("grinderEditor.conical")
          : g.burrType === "flat"
            ? t("grinderEditor.flat")
            : "";
      const settingTypeLabel =
        g.settingType === "preset"
          ? t("grinderEditor.positions")
          : t("grinderEditor.stepless");
      tile.innerHTML = `
  ...
  document
    .getElementById("btn-grinder-picker-cancel")
    ?.addEventListener("click", () => {
      const modal = document.getElementById("grinder-picker-modal");
      if (modal) modal.hidden = true;
    });
  ...
  document
    .getElementById("grinder-picker-modal")
    ?.addEventListener("click", (e) => {
  ...
  document
    .getElementById("btn-edit-pick-grinder")
    ?.addEventListener("click", openGrinderPickerModal);

  /* ── Bohnen Modal ─────────────────────────────────────── */

  const batchAddModalEl = document.getElementById("batch-add-modal");
  const batchDatePickerModalEl = document.getElementById(
    "batch-date-picker-modal",
  );
  const _getMonthName = (month1Based) =>
    new Intl.DateTimeFormat("en-US", { month: "long" }).format(
      new Date(2000, month1Based - 1),
    );
  ...
  function createBeanTile(bean, onBeanClick) {
    ...
    tile.className = "bean-tile";
    const varietyStr =
      Array.isArray(bean.variety) && bean.variety.length
        ? bean.variety.join(", ")
        : "—";
    const altitudeStr =
      Array.isArray(bean.altitude) && bean.altitude.length === 2
  ...
  `;
    tile.addEventListener("click", () => {
      if (onBeanClick) onBeanClick(bean);
    });
    return tile;
  }
  ...
  function renderBeanFolderView(roasterMap, ungrouped, panelEl, onBeanClick) {
    ...
    const sorted = [...roasterMap.entries()].sort((a, b) =>
  ...
  function renderBeanRoasterView(
    roaster,
    beans,
    roasterMap,
    ungrouped,
  ...
  `;
    backBar
      .querySelector(".bean-back-btn")
      .addEventListener("click", () =>
        renderBeanFolderView(roasterMap, ungrouped, panelEl, onBeanClick),
      );
  ...
  function renderBeanTiles(beans) {
    ...
    if (!Array.isArray(beans) || beans.length === 0) {
      panel.innerHTML =
        '<div class="bohnen-empty-state">No beans available</div>';
      return;
    }
  ...
  function _beanPickerOptions() {
    ...
    return {
      ...
      "bean-variety": uniq(
        NSXCore.getBeans().flatMap((b) =>
          Array.isArray(b.variety) ? b.variety : [],
        ),
      ),
    };
  }
  ...
  function _renderFieldPickerList(filter) {
    ...
    const filtered = q
      ? _fieldPickerAllOptions.filter((o) => o.toLowerCase().includes(q))
      : _fieldPickerAllOptions;
  ...
  function _fpKbSetShift(on) {
    ...
    document
      .querySelectorAll(".fp-keyboard .fp-key[data-key]")
      .forEach((btn) => {
        const k = btn.dataset.key;
        if (/^[a-z]$/.test(k))
          btn.textContent = _fpKbShift ? k.toUpperCase() : k.toLowerCase();
      });
  }
  ...
  function _setupKeyboard(keyboardId, shiftBtnId, bsBtnId) {
    ...
    kb.addEventListener("pointerdown", (e) => {
      ...
      if (!key) return;
      if (key.id === shiftBtnId) {
        _fpKbSetShift(!_fpKbShift);
        return;
      }
      if (key.id === bsBtnId || key.classList.contains("fp-key--bs")) {
        _fpKbBackspace();
        _fpKbBsTimer = setTimeout(() => {
          _fpKbBsInterval = setInterval(_fpKbBackspace, 80);
        }, 400);
        return;
      }
  ...
  document
    .getElementById("btn-text-editor-cancel")
    ?.addEventListener("click", () => closeTextEditorModal(false));
  document
  ...
  function openFieldPicker(
    inputEl,
    options,
    { inputMode = "text", onConfirm = null, initialValue = null } = {},
  ) {
    ...
    document
      .getElementById("field-picker-keyboard")
      ?.classList.toggle("fp-keyboard--numeric", inputMode === "numeric");
    pickerInput.value =
      initialValue !== null ? String(initialValue) : inputEl?.value || "";
  ...
  document
    .getElementById("btn-field-picker-cancel")
    ?.addEventListener("click", () => closeFieldPicker(false));
  document
    .getElementById("btn-field-picker-confirm")
    ?.addEventListener("click", () => closeFieldPicker(true));
  document
    .getElementById("field-picker-input")
    ?.addEventListener("input", (e) => _renderFieldPickerList(e.target.value));

  document
    .getElementById("profile-picker-search")
  ...
  document
    .querySelector(".workflows-search")
    ?.addEventListener("click", (e) => {
      openFieldPicker(e.target, []);
    });
    ...
    for (let i = 0; i < _NP_SLOTS; i++) {
      ...
      if (vi < 0 || vi >= _npValues.length || Math.abs(angle) >= 90) {
        ...
        const fontSize = Math.round(
          36 * (0.6 + 0.9 * Math.pow(Math.cos(rad), 12)),
        );
        items[i].style.visibility = "";
        items[i].style.fontSize = `${fontSize}px`;
        items[i].textContent = _npFormatter
          ? _npFormatter(_npValues[vi])
          : _npValues[vi].toFixed(_npDecimalPlaces);
        items[i].style.transform =
          `translateY(${y}px) perspective(600px) rotateX(${(-angle).toFixed(2)}deg)`;
      }
    }
  }
  ...
  function _npSnap(indexFloat) {
    ...
    const target = Math.max(
      0,
      Math.min(_npValues.length - 1, Math.round(indexFloat)),
    );
    const spring = () => {
  ...
  function _npMomentum(velocityPxPerMs) {
    ...
    const tick = (now) => {
      ...
      if (_npCurrentFloat <= 0) {
        _npCurrentFloat = 0;
        vel = 0;
      }
      if (_npCurrentFloat >= _npValues.length - 1) {
        _npCurrentFloat = _npValues.length - 1;
        vel = 0;
      }
  ...
  /* ── Text Picker ─────────────────────────────────────── */
  {
    ...
    window._openTextPicker = function (currentValue, onConfirm) {
      ...
      setTimeout(() => {
        _textPickerInput.focus();
        _textPickerInput.select();
      }, 80);
    };

    document
      .getElementById("btn-text-picker-cancel")
      ?.addEventListener("click", () => {
    ...
    document
      .getElementById("btn-text-picker-confirm")
      ?.addEventListener("click", () => {
    ...
    _textPickerInput?.addEventListener("keydown", (e) => {
      if (e.key === "Enter")
        document.getElementById("btn-text-picker-confirm")?.click();
      if (e.key === "Escape")
        document.getElementById("btn-text-picker-cancel")?.click();
    });
  }
  ...
  function closeNumberPicker(confirm) {
    if (_npAnimFrame) {
      cancelAnimationFrame(_npAnimFrame);
      _npAnimFrame = null;
    }
    const modal = document.getElementById("number-picker-modal");
    if (confirm && _npOnConfirm) {
      const idx = Math.max(
        0,
        Math.min(_npValues.length - 1, Math.round(_npCurrentFloat)),
      );
  ...
  document
    .getElementById("btn-number-picker-cancel")
    ?.addEventListener("click", () => closeNumberPicker(false));
  document
    .getElementById("btn-number-picker-confirm")
    ?.addEventListener("click", () => closeNumberPicker(true));
...

  {
    ...
    if (wrap) {
      ...
      wrap.addEventListener("pointermove", (e) => {
        ...
        _npLastY = e.clientY;
        _npLastTime = now;
        _npCurrentFloat = Math.max(
          0,
          Math.min(_npValues.length - 1, _npDragStartFloat + dy / _NP_ITEM_H),
        );
        _npRender(_npCurrentFloat);
      });
...

  {
    ...
    const profileEditorTextInputs = {
      "profile-editor-title": () =>
        uniq(allProfiles().map((r) => r.profile?.title)),
      "profile-editor-author": () =>
        uniq(allProfiles().map((r) => r.profile?.author)),
    };
...

  {
    const numberPickerInputs = {
      "bean-altitude-min": () =>
        openNumberPicker(
          _npMakeRange(0, 4000, 100),
          parseFloat(document.getElementById("bean-altitude-min")?.value) || 0,
          (v) => {
            document.getElementById("bean-altitude-min").value = v;
          },
        ),
      "bean-altitude-max": () =>
        openNumberPicker(
          _npMakeRange(0, 4000, 100),
          parseFloat(document.getElementById("bean-altitude-max")?.value) || 0,
          (v) => {
            document.getElementById("bean-altitude-max").value = v;
          },
        ),
      "grinder-burr-size": () =>
        openNumberPicker(
          _npMakeRange(20, 140, 1),
          parseFloat(document.getElementById("grinder-burr-size")?.value) || 64,
          (v) => {
            document.getElementById("grinder-burr-size").value = v;
          },
        ),
    };
  ...
  function _batchDateDraftToDate() {
    ...
    return new Date(
      Date.UTC(
        _batchDateDraft.year,
        _batchDateDraft.month - 1,
        _batchDateDraft.day,
        12,
        0,
        0,
  ...
  function _renderBatchDatePicker() {
    ...
    if (previewEl)
      previewEl.textContent = _formatBatchDateDisplayValue(value) || "—";
    if (monthEl)
      monthEl.textContent = _getMonthName(_batchDateViewMonth) || "—";
    if (yearEl)
      yearEl.textContent = String(_batchDateViewYear || _batchDateDraft.year);
    if (!daysEl) return;

    const firstOfMonth = new Date(
      Date.UTC(_batchDateViewYear, _batchDateViewMonth - 1, 1, 12, 0, 0),
    );
    const daysInMonth = new Date(
      Date.UTC(_batchDateViewYear, _batchDateViewMonth, 0, 12, 0, 0),
    ).getUTCDate();
    ...
    for (let idx = 0; idx < startOffset; idx++) {
      cells.push(
        '<span class="batch-date-picker-day batch-date-picker-day--empty" aria-hidden="true"></span>',
      );
    }
    for (let day = 1; day <= daysInMonth; day++) {
      const isSelected =
        _batchDateDraft.year === _batchDateViewYear &&
        _batchDateDraft.month === _batchDateViewMonth &&
        _batchDateDraft.day === day;
      const isToday =
        todayYear === _batchDateViewYear &&
        todayMonth === _batchDateViewMonth &&
        todayDay === day;
      const className = [
        ...
        .filter(Boolean)
        .join(" ");
      const valueStr = _formatBatchDateValue({
        year: _batchDateViewYear,
        month: _batchDateViewMonth,
        day,
      });
      cells.push(
        `<button type="button" class="${className}" data-batch-date-value="${valueStr}" role="gridcell" aria-selected="${isSelected ? "true" : "false"}">${day}</button>`,
      );
    }
  ...
  function openBatchDatePickerModal(onApply = null, currentIso = null) {
    ...
    const current = _parseBatchDateValue(
      currentIso ?? _getBatchRoastDateValue(),
    );
    if (current) {
  ...
  // Debug: log the age for a test batch (comment out later)
  // console.log('DEBUG: Test-Batch age:', formatBatchAge('2025-04-20'));

  function renderBatchList(batches) {
    _lastBatches = (Array.isArray(batches) ? batches : [])
      .slice()
      .sort((a, b) => {
  ...
  async function loadAndRenderBatches(beanId) {
    ...
    try {
      ...
      renderBatchList(
        Array.isArray(batches) ? batches : (batches?.items ?? []),
      );
    } catch {
      if (listEl)
  ...
  function openBatchModal(batch = null) {
    ...
    _setBatchRoastDateValue(
      batch?.roastDate ? batch.roastDate.slice(0, 10) : "",
    );
    document.getElementById("batch-roast-level").value =
      batch?.roastLevel ?? "";
    ...
    document.getElementById("batch-quality-score").value =
      batch?.qualityScore ?? "";
    ...
    if (titleEl)
      titleEl.textContent = isEditing
        ? t("batchEditor.editTitle")
        : t("batchEditor.title");
    const saveBtn = document.getElementById("btn-batch-save");
    if (saveBtn)
      saveBtn.textContent = isEditing ? t("action.save") : t("action.add");
  ...
  ["pointerdown", "pointerup", "touchstart", "touchend", "click"].forEach(
    (eventName) => {
      batchAddModalEl?.addEventListener(
        eventName,
        (e) => {
          e.stopPropagation();
        },
      ...
      batchDatePickerModalEl?.addEventListener(
        eventName,
        (e) => {
          e.stopPropagation();
        },
  ...
  document
    .getElementById("btn-batch-add")
    ?.addEventListener("click", () => openBatchModal(null));
  document
    .getElementById("batch-roast-date")
    ?.addEventListener("click", () => openBatchDatePickerModal());
  ...
  document
    .getElementById("btn-batch-date-clear")
    ?.addEventListener("click", () => {
      _setBatchRoastDateValue("");
      if (batchDatePickerModalEl) batchDatePickerModalEl.hidden = true;
    });
  document
    .getElementById("btn-batch-date-month-down")
    ?.addEventListener("click", () => _shiftBatchDateView("month", -1));
  document
    .getElementById("btn-batch-date-month-up")
    ?.addEventListener("click", () => _shiftBatchDateView("month", 1));
  document
    .getElementById("btn-batch-date-year-down")
    ?.addEventListener("click", () => _shiftBatchDateView("year", -1));
  document
    .getElementById("btn-batch-date-year-up")
    ?.addEventListener("click", () => _shiftBatchDateView("year", 1));
  document
    .getElementById("batch-date-picker-days")
    ?.addEventListener("click", (e) => {
  ...
  batchDatePickerModalEl?.addEventListener("click", (e) => {
    if (e.target === batchDatePickerModalEl)
      batchDatePickerModalEl.hidden = true;
  });

  document
    .getElementById("batch-list")
    ?.addEventListener("click", async (e) => {
  ...
  document
    .getElementById("btn-batch-delete")
    ?.addEventListener("click", async () => {
  ...
  document
    .getElementById("btn-batch-save")
    ?.addEventListener("click", async () => {
      if (!_editingBean?.id) return;

      const roastDateRaw = _getBatchRoastDateValue();
      const roastLevel =
        document.getElementById("batch-roast-level")?.value.trim() || undefined;
      const weightRaw = document.getElementById("batch-weight")?.value;
      const priceRaw = document.getElementById("batch-price")?.value;
      const currency =
        document.getElementById("batch-currency")?.value.trim() || undefined;
      const qualityRaw = document.getElementById("batch-quality-score")?.value;
      const notes =
        document.getElementById("batch-notes")?.value.trim() || undefined;

      const payload = {};
      if (roastDateRaw)
        payload.roastDate = _batchDateValueToApiIso(roastDateRaw);
      ...
      if (qualityRaw !== "") {
        ...
        if (Number.isFinite(q)) payload.qualityScore = q;
      }
      ...
      try {
        ...
        if (saveBtn)
          saveBtn.textContent = _editingBatch?.id
            ? t("action.save")
            : t("action.add");
        if (batchAddModalEl) batchAddModalEl.hidden = true;
        showToast(
          _editingBatch?.id ? t("toast.batchSaved") : t("toast.batchAdded"),
        );
        loadAndRenderBatches(_editingBean.id);
        if (
          beanManagerModalEl &&
          !beanManagerModalEl.hidden &&
          _beanManagerSelectedBean?.id
        ) {
  ...
  function _persistBeanManagerCollapsedRoasters() {
    patchStoreSettings({
      nsx_bean_manager_collapsed_roasters: [..._beanManagerCollapsedRoasters],
    });
  }
  ...
  function _beanManagerRenderList() {
    ...
    for (const [roaster, members] of groups) {
      ...
      header.type = "button";
      header.className =
        "profile-picker-group-header" + (collapsed ? " is-collapsed" : "");
      ...
      if (!collapsed) {
        for (const bean of members) {
          ...
          item.role = "option";
          item.className =
            "profile-picker-item is-grouped" +
            (bean.id === _beanManagerSelectedBean?.id ? " is-selected" : "");
          if (bean.archived) item.classList.add("bean-manager-item-archived");
          const sub = [bean.processing, bean.country]
            .filter(Boolean)
            .join(" · ");
  ...
  function _beanManagerSuggestions(field) {
    ...
    if (field === "variety") {
      return [
        ...new Set(
          NSXCore.getBeans()
            .flatMap((b) => (Array.isArray(b.variety) ? b.variety : []))
            .filter(Boolean),
        ),
      ].sort((a, b) => a.localeCompare(b, "en"));
    }
  ...
  function _beanManagerApplyField(target, field, value) {
    if (field === "variety") {
      ...
      const max = Array.isArray(target.altitude)
        ? (target.altitude[1] ?? n)
        : n;
      target.altitude = Number.isFinite(n) ? [n, max] : undefined;
    } else if (field === "altMax") {
      const n = Number(value);
      const min = Array.isArray(target.altitude)
        ? (target.altitude[0] ?? n)
        : n;
  ...
  function _beanManagerRenderDetail(bean) {
    ...
    if (titleEl)
      titleEl.textContent = bean._draft
        ? t("beanEditor.title")
        : bean.roaster
    ...
    if (fieldsEl)
      fieldsEl.classList.toggle("is-readonly", !!_beanManagerPickCallback);
    if (fieldsEl) {
      const varietyVal =
        Array.isArray(bean.variety) && bean.variety.length
          ? bean.variety.join(", ")
          : "";
      const altMin = Array.isArray(bean.altitude)
        ? (bean.altitude[0] ?? "")
        : "";
      const altMax = Array.isArray(bean.altitude)
        ? (bean.altitude[1] ?? "")
        : "";
      ...
      ${
        show || !isEmpty(bean.notes)
          ? `<button type="button" class="bean-manager-prop-tile bean-manager-prop-tile--full" data-bm-field="notes" data-bm-value="${_esc(String(bean.notes ?? ""))}">
    ...
    const batchesSection = document.querySelector(
      ".bean-manager-batches-section",
    );
    if (bean.id) {
  ...
  function _beanManagerRenderBatches(batches) {
    ...
    const archiveChk = document.getElementById(
      "chk-bean-manager-batch-show-archived",
    );
    if (archiveChk) archiveChk.checked = _beanManagerShowArchivedBatches;

    const visible = _beanManagerShowArchivedBatches
      ? sorted
      : sorted.filter((b) => !b.archived);
    if (visible.length === 0) {
      listEl.innerHTML = `<div class="batch-empty">${t("beanEditor.noBatches")}</div>`;
      return;
    }
    ...
    listEl.innerHTML = visible
      .map((b) => {
        ...
        const weightRemStr =
          b.weightRemaining != null ? `${b.weightRemaining} g` : "—";
  ...
  async function _beanManagerLoad() {
    const listEl = document.getElementById("bean-manager-list");
    if (listEl)
      listEl.innerHTML = `<div class="bohnen-empty-state bean-manager-empty">${t("status.loading")}</div>`;
    try {
      ...
      if (_beanManagerSelectedBean) {
        _beanManagerSelectedBean =
          beans.find((b) => b.id === _beanManagerSelectedBean.id) ?? null;
      }
      if (!_beanManagerSelectedBean && !_beanManagerPickCallback) {
        if (_beanManagerAutoSelectId) {
          _beanManagerSelectedBean =
            beans.find((b) => b.id === _beanManagerAutoSelectId) ?? null;
        }
        if (!_beanManagerSelectedBean) {
      ...
      if (listEl)
        listEl.innerHTML = `<div class="bohnen-empty-state bean-manager-empty">${t("status.loadFailed")}</div>`;
    }
  }

  function openBeanManagerModal(pickCallback = null) {
    if (!beanManagerModalEl) return;
    _beanManagerPickCallback =
      typeof pickCallback === "function" ? pickCallback : null;
    ...
    if (beanModalTitleEl)
      beanModalTitleEl.textContent = pickCallback
        ? t("beanManager.pickTitle")
        : t("beanList.title");
    const placeholderEl = document.getElementById("bean-manager-placeholder");
    if (placeholderEl) placeholderEl.hidden = !!pickCallback;
    document.getElementById("bean-manager-detail") &&
      (document.getElementById("bean-manager-detail").hidden = true);
  ...
  document
    .getElementById("btn-bohnen")
    ?.addEventListener("click", () => openBeanManagerModal());

  document
    .getElementById("btn-bean-manager-close")
    ?.addEventListener("click", () => {
      if (beanManagerModalEl) beanManagerModalEl.hidden = true;
    });
  ...
  document
    .getElementById("bean-manager-search")
    ?.addEventListener("pointerdown", (e) => {
      e.preventDefault();
  ...
  document
    .getElementById("bean-manager-list")
    ?.addEventListener("click", (e) => {
  ...
  document
    .getElementById("btn-bean-manager-new")
    ?.addEventListener("click", () => {
  ...
  document
    .getElementById("chk-bean-manager-toggle-archive")
    ?.addEventListener("change", (e) => {
  ...
  document
    .getElementById("bean-manager-detail")
    ?.addEventListener("pointerdown", (e) => {
      const target =
        e.target.closest(".bean-manager-prop-split-half") ||
        e.target.closest(".bean-manager-prop-tile[data-bm-field]");
      if (!target) return;
      e.preventDefault();
    ...
    ?.addEventListener("click", async (e) => {
      ...
      if (e.target.closest("#btn-bm-save-new")) {
        ...
        try {
          ...
          const newBean =
            NSXCore.getBeans().find((b) => b.id === created?.id) ?? null;
      ...
      if (e.target.closest("#btn-bm-archive")) {
        ...
        try {
          if (isArchived) {
            await unarchiveBean(bean.id, bean);
            showToast(t("toast.beanUnarchived"));
          } else {
            await archiveBean(bean.id, bean);
            showToast(t("toast.beanArchived"));
          }
          _beanManagerLoad();
        } catch (err) {
          showToast(t("toast.error") + ": " + err.message);
      ...
      if (e.target.closest("#btn-bm-delete")) {
        ...
        if (
          !(await showConfirm(
            t("confirm.deleteBean").replace(
              "{name}",
              _beanManagerSelectedBean.name || t("beanEditor.unnamed"),
            ),
          ))
        )
          return;
  ...
  document
    .getElementById("btn-bean-manager-batch-add")
    ?.addEventListener("click", () => {
      if (!_beanManagerSelectedBean?.id) return;
  ...
  document
    .getElementById("bean-manager-batch-list")
    ?.addEventListener("click", async (e) => {
      // Action buttons
      ...
      if (actionBtn) {
        ...
        if (action === "edit") {
          ...
          try {
            ...
            if (_beanManagerSelectedBean?.id)
              _beanManagerLoadBatches(_beanManagerSelectedBean.id);
          } catch (err) {
            showToast(t("toast.error") + ": " + err.message);
          }
  ...
  document
    .getElementById("chk-bean-manager-batch-show-archived")
    ?.addEventListener("change", (e) => {
  ...
  async function loadAndRenderGrinders() {
    const panel = document.getElementById("muehlen-panel-list");
    if (panel)
      panel.innerHTML = `<div class="bohnen-empty-state">${t("status.loading")}</div>`;
    try {
      const grindersRes = await fetchGrinders();
      const gList = Array.isArray(grindersRes)
        ? grindersRes
        : (grindersRes?.items ?? []);
      ...
      if (panel)
        panel.innerHTML = '<div class="bohnen-empty-state">Error loading</div>';
    }
  }
  ...
  document
    .getElementById("grinder-setting-type-toggle")
    ?.addEventListener("click", (e) => {
  ...
  function openGrinderDetailModal(grinder = null) {
    ...
    if (titleEl)
      titleEl.textContent = grinder
        ? t("grinderEditor.editTitle")
        : t("grinderEditor.title");

    document.getElementById("grinder-model").value = grinder?.model ?? "";
    document.getElementById("grinder-burrs").value = grinder?.burrs ?? "";
    document.getElementById("grinder-burr-size").value =
      grinder?.burrSize ?? "";
    document.getElementById("grinder-burr-type").value =
      grinder?.burrType ?? "";
    document.getElementById("grinder-small-step").value =
      grinder?.settingSmallStep ?? "";
    document.getElementById("grinder-big-step").value =
      grinder?.settingBigStep ?? "";
    ...
    document
      .getElementById("grinder-model")
      ?.closest(".bean-field-row")
  ...
  document
    .getElementById("btn-muehlen-close")
    ?.addEventListener("click", () => {
      if (muehlenModalEl) muehlenModalEl.hidden = true;
    });

  document
    .getElementById("btn-muehlen-new")
    ?.addEventListener("click", () => openGrinderDetailModal(null));
  ...
  document
    .getElementById("btn-grinder-delete")
    ?.addEventListener("click", async () => {
      ...
      try {
        ...
        if (muehlenCreateModalEl) muehlenCreateModalEl.hidden = true;
        showToast(
          t("toast.grinderDeleted").replace("{name}", _editingGrinder.model),
        );
        loadAndRenderGrinders();
      } catch (err) {
  ...
  document
    .getElementById("btn-muehlen-save")
    ?.addEventListener("click", async () => {
      ...
      const burrs =
        document.getElementById("grinder-burrs")?.value.trim() || undefined;
      const burrSizeRaw = document.getElementById("grinder-burr-size")?.value;
      const burrType =
        document.getElementById("grinder-burr-type")?.value || undefined;

      const activeTypeBtn = document.querySelector(
        "#grinder-setting-type-toggle .grinder-toggle-btn.is-active",
      );
      ...
      const presetsRaw =
        document.getElementById("grinder-setting-values")?.value.trim() ?? "";

      const payload = { model, settingType };
      if (burrs) payload.burrs = burrs;

      const burrSize = burrSizeRaw !== "" ? Number(burrSizeRaw) : null;
      if (Number.isFinite(burrSize) && burrSize > 0)
        payload.burrSize = burrSize;
  ...
  document
    .getElementById("btn-steam-overlay-stop")
    ?.addEventListener("click", () => {
      setMachineState?.("idle").catch(() => {});
    });

  document
    .getElementById("btn-steam-overlay-back")
    ?.addEventListener("click", () => {
      const overlayEl = document.getElementById("steam-overlay");
      const cornerEl = document.getElementById("steam-corner");
      if (overlayEl) overlayEl.hidden = true;
  ...
  document
    .getElementById("btn-steam-corner-stop")
    ?.addEventListener("click", () => {
      setMachineState?.("idle").catch(() => {});
    });

  document
    .getElementById("btn-steam-corner-graph")
    ?.addEventListener("click", () => {
      const cornerEl = document.getElementById("steam-corner");
      const overlayEl = document.getElementById("steam-overlay");
      const graphEl = document.getElementById("steam-overlay-graph");
      ...
      if (cornerEl) cornerEl.hidden = true;
      const steamDuration = NSXCore.getSteamDuration();
      if (targetEl && steamDuration > 0)
        targetEl.textContent = `/ ${steamDuration} s`;
      if (progressEl && steamDuration > 0) {
        const sec = steamSession
          ? Math.floor((Date.now() - steamSession.startTime) / 1000)
          : 0;
  ...
  document
    .getElementById("btn-hotwater-overlay-stop")
    ?.addEventListener("click", () => {
      setMachineState?.("idle").catch(() => {});
    });

  document
    .getElementById("btn-flush-overlay-stop")
    ?.addEventListener("click", () => {
      setMachineState?.("idle").catch(() => {});
    });

  document
    .getElementById("btn-needswater-overlay-stop")
    ?.addEventListener("click", () => {
      setMachineState?.("idle").catch(() => {});
    });

  document
    .getElementById("btn-espresso-fs-exit")
    ?.addEventListener("click", () => {
      if (NSXCore.getMachineState() === "espresso") {
        setMachineState?.("idle").catch(() => {});
        return;
  ...
  document
    .getElementById("btn-espresso-fs-skip-step")
    ?.addEventListener("click", () => {
      if (NSXCore.getMachineState() !== "espresso") {
        showToast(t("toast.skipStepOnly"));
        return;
