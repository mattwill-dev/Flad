...
(() => {
  ...
  const homeWorkflowRoasterEl = document.getElementById(
  ...
  const homeWorkflowSettingEl = document.getElementById(
    "home-workflow-setting",
  );
  const homeWorkflowProfileEl = document.getElementById(
    "home-workflow-profile",
  );
  ...
  const homeWorkflowBeverageEl = document.getElementById(
    "home-workflow-beverage",
  );
  const homeWorkflowRatioEl = document.getElementById("home-workflow-ratio");
  const headerStatusBadgeEl = document.getElementById("header-status-badge");
  const headerStatusTextEl = document.getElementById("header-status-text");
  const homeWorkflowLastShotDateEl = document.getElementById(
    "home-workflow-last-shot-date",
  );
  const homeWorkflowLastShotDurationEl = document.getElementById(
    "home-workflow-last-shot-duration",
  );
  ...
  const waterRefillLineEl = document.getElementById("water-refill-line");
  ...
  const steamTemperatureOrbEl = document.getElementById(
    "steam-temperature-orb",
  );
  ...
  function _tempToGaugePct(temp) {
    ...
    return (
      _gaugeSplit +
      ((1 - _gaugeSplit) * (t - _gaugeMid)) / (_gaugeMax - _gaugeMid)
    );
  }
  ...
  function _resampleSeries(refX, refY, targetX) {
    ...
    for (let i = 0; i < targetX.length; i++) {
      const t = targetX[i];
      if (!Number.isFinite(t) || t < refX[0] || t > refX[n - 1]) {
        out[i] = null;
        continue;
      }
      ...
      if (!Number.isFinite(y0)) {
        out[i] = Number.isFinite(y1) ? y1 : null;
        continue;
      }
      out[i] =
        x1 === x0 || !Number.isFinite(y1)
          ? y0
          : y0 + (y1 - y0) * ((t - x0) / (x1 - x0));
    }
    return out;
  ...
  function getSeriesVisibility(overrides, mode = "workflow") {
    const base =
      mode === "live"
        ? LIVE_SERIES_VISIBILITY
        : mode === "history"
          ? HISTORY_SERIES_VISIBILITY
  ...
  function setSeriesVisibility(mode, visibility) {
    ...
    window.dispatchEvent(
      new CustomEvent("ui:seriesVisibilityChanged", {
        detail: { mode, visibility: { ...resolved } },
      }),
    );
  }
  ...
  function showStateToast(msg, ms = 2500) {
    ...
    stateToastTimer = setTimeout(
      () => stateToastEl.classList.remove("show"),
      ms,
    );
  }
  ...
  function setMachineConnected(connected) {
    if (machineStatusPillEl) {
      machineStatusPillEl.textContent = connected
        ? t("status.connected")
  ...
  function setMachineStateText(state) {
    if (machineStatusTextEl) machineStatusTextEl.textContent = state;
  }
  ...
  function setScaleConnected(connected) {
    scaleStatusPillEl.textContent = connected
      ? t("status.connected")
      : t("status.search");
    scaleStatusPillEl.className = connected
      ? "status-pill status-pill-connected"
      : "status-pill status-pill-disconnected";
    scaleTileEl?.classList.toggle("is-connected", connected);
    if (scaleTareButtonEl) {
      scaleTareButtonEl.disabled = false;
      scaleTareButtonEl.classList.toggle(
        "scale-plate-btn--offline",
        !connected,
      );
    }
  ...
  function setBrewGroupTemperature(value, target = TARGET_BREW_TEMPERATURE) {
    ...
    if (
      headerStatusBadgeEl &&
      headerStatusTextEl &&
      !headerStatusBadgeEl.dataset.stateLock
    ) {
      headerStatusBadgeEl.classList.toggle(
        "app-header-status--heating",
        !isReady,
      );
      headerStatusTextEl.textContent = `${formatTemperature(value)}°  ${isReady ? t("status.ready") : t("status.heating")}`;
    }
  ...
  function renderWorkflows(workflows, selectedIndex) {
    ...
    <li class="recipe-item workflow-card${isActive ? " workflow-card-active" : ""}${isPending ? " workflow-card-pending" : ""}" data-workflow-index="${origIdx}">
      ...
      <div class="workflow-swipe-layer">
        <button type="button" class="workflow-edit-btn" data-edit-index="${origIdx}" aria-label="Edit recipe">${editIconSvg}</button>
  ...
  function setWorkflowSyncState(state) {
    const dots = [
      _syncDotEl,
      document.querySelector(".home-rr-sync-dot"),
    ].filter(Boolean);
  ...
  function setCurrentWorkflow(workflow) {
    const widget = document.getElementById("home-active-recipe-card");
    if (!workflow) {
    ...
    if (homeWorkflowSettingEl)
      homeWorkflowSettingEl.textContent = workflow.grinderSetting;
    if (homeGrinderNameEl)
      homeGrinderNameEl.textContent = workflow.grinderModel || "—";
    if (homeGrindSettingEl)
      homeGrindSettingEl.textContent = workflow.grinderSetting || "—";
    if (homeBeanNameEl) homeBeanNameEl.textContent = workflow.coffeeName || "—";
    if (homeWorkflowProfileEl)
      homeWorkflowProfileEl.textContent = workflow.profileTitle;
    if (homeWorkflowDoseEl)
      homeWorkflowDoseEl.innerHTML =
        workflow.targetDoseWeight > 0
          ? `${workflow.targetDoseWeight}<span class="hc-param-unit">g</span>`
          : "—";
    if (homeWorkflowTempEl) {
      const g = Number(workflow.groupTemp);
      homeWorkflowTempEl.textContent =
        workflow.profileTemp || (g > 0 ? `${g}°C` : "—");
    }
    if (homeWorkflowBeverageEl) {
      const yield_ = Number(workflow.targetYield);
      homeWorkflowBeverageEl.innerHTML =
  ...
  function setWaterRefillLevel(mm) {
    ...
    waterRefillLevelMm = Number.isFinite(value)
      ? Math.max(5, Math.min(value, WATER_TANK_MAX_MM))
      : null;
    ...
    if (waterRefillLineEl) {
      if (Number.isFinite(waterRefillLevelMm)) {
        const pct = Math.min(
          100,
          (waterRefillLevelMm / WATER_TANK_MAX_MM) * 100,
        );
        waterRefillLineEl.style.bottom = `${pct.toFixed(1)}%`;
        waterRefillLineEl.hidden = false;
      } else {
        waterRefillLineEl.hidden = true;
      }
    }
  ...
  function setWaterLevel(mm) {
    ...
    const fillPct = Math.min(
      (smoothedMl / (WATER_TANK_MAX_MM * ML_PER_MM)) * 100,
      100,
    );
    if (waterGaugeFillEl) waterGaugeFillEl.style.height = `${fillPct}%`;
    if (headerWaterOrbEl)
      headerWaterOrbEl.style.setProperty(
        "--temperature-progress",
        `${fillPct.toFixed(1)}%`,
      );
    if (headerWaterPctEl) {
      headerWaterPctEl.textContent =
        _waterDisplayUnit === "ml"
          ? `${smoothedMl} ml`
          : `${Math.round(fillPct)}%`;
      // Tracks the top of the fill (10px above its surface) instead of a fixed
      // spot, so the number rides down with the level as the tank drains.
      // Clamped shy of 100% so it can't clip past the gauge's top edge.
      const anchorPct = Math.min(fillPct, 90);
      headerWaterPctEl.style.bottom = `calc(${anchorPct.toFixed(1)}% + 10px)`;
    }
    updateWaterWarningState();
  ...
  function setSteamTemperatureOrb(value) {
    ...
    const progress = isReady
      ? 100
      : Math.max(0, Math.min(v / currentSteamTarget, 1)) * 100;
    const orbStateClass = isReady
      ? "temperature-orb-ready"
      : isWarmup
        ? "temperature-orb-warmup"
    ...
    steamTemperatureOrbEl.style.setProperty(
      "--temperature-progress",
      `${progress.toFixed(1)}%`,
    );
  }
  ...
  function setHotwaterWidget(temperature, flowRate, volume) {
    if (hotwaterTempEl)
      hotwaterTempEl.innerHTML = `${Number(temperature).toFixed(0)}<span class="hc-param-unit hc-param-unit--deg">°</span>`;
    if (hotwaterFlowEl)
      hotwaterFlowEl.innerHTML = `${Number(flowRate).toFixed(1)}<span class="hc-param-unit">ml/s</span>`;
    if (hotwaterVolumeEl)
      hotwaterVolumeEl.innerHTML = `${Number(volume).toFixed(0)}<span class="hc-param-unit">ml</span>`;
  }
  ...
  function updateRecipeListFade() {
    const maxScrollTop =
      recipeListScrollEl.scrollHeight - recipeListScrollEl.clientHeight;
    const showTopFade = recipeListScrollEl.scrollTop > 6;
    const showBottomFade =
      maxScrollTop > 6 && recipeListScrollEl.scrollTop < maxScrollTop - 6;
  ...
  function _legendHostFor(graphEl) {
    if (graphEl?.id === "shot-review-graph") {
      return (
        document.getElementById("shot-review-legend-host") ||
  ...
  function renderShotLegend(graphEl, visibility, onToggle, navContext) {
    ...
    legend.innerHTML = [
      ...
      createLegendItem(
        "scaleRate",
        t("legend.weightFlow"),
        CHART_COLORS.weightRate,
        visibility.scaleRate,
      ),
      createLegendItem(
        "temperature",
        t("legend.temperature"),
  ...
  function updateWorkflowLegendLive(graphEl, vals) {
    ...
    if (!legend) return;
    const fmt = {
      pressure: Number.isFinite(vals.pressure)
        ? `${vals.pressure.toFixed(1)} bar`
        : "— bar",
      flow: Number.isFinite(vals.flow)
        ? `${vals.flow.toFixed(1)} ml/s`
        : "— ml/s",
      scaleRate:
        Number.isFinite(vals.scaleRate) && vals.scaleRate >= 0
          ? `${vals.scaleRate.toFixed(1)} g/s`
          : "— g/s",
      temperature:
        Number.isFinite(vals.temperature) && vals.temperature > 0
          ? `${vals.temperature.toFixed(1)}°C`
          : "—°C",
    };
    legend
      .querySelectorAll(".workflow-legend-item[data-series-key]")
      .forEach((btn) => {
  ...
  function renderShotMeta(reserveEl, shot, normalized, navContext, workflow) {
    ...
    const hasValidDate =
      shotDate instanceof Date && !Number.isNaN(shotDate.getTime());

    const day = hasValidDate
      ? new Intl.DateTimeFormat("en-US", { weekday: "short" }).format(shotDate)
      : "--";
    ...
    const totalDuration =
      Array.isArray(normalized?.elapsed) && normalized.elapsed.length
    ...
    const dose =
      Number(
        shot?.annotations?.actualDoseWeight || ctx.targetDoseWeight || 0,
      ) || null;
    ...
    const annYield = Number(
      shot?.annotations?.actualYield ?? shot?.annotations?.extras?.actualYield,
    );
    if (Number.isFinite(annYield) && annYield > 0) {
      ...
      if (Number.isFinite(snapVol) && snapVol > 0) {
        ...
        if (actualOut === null)
          actualOut = integrateScaleRate(
            normalized?.elapsed,
            normalized?.scaleRate,
          );
        if (
          actualOut === null &&
          shot.annotations?.extras?.virtualScale === true
        ) {
    ...
    const outStr =
      actualOut != null
        ? `${isEstimatedOut ? "~" : ""}${isEstimatedOut ? Math.round(actualOut) : actualOut.toFixed(1)}${outUnit}`
        : "—";
    const ratioStr =
      dose != null && actualOut != null && dose > 0
  ...
  `;
    ...
    meta
      .querySelector('[data-nav="older"]')
      ?.addEventListener("click", navContext?.onOlder);
    meta
      .querySelector('[data-nav="newer"]')
      ?.addEventListener("click", navContext?.onNewer);
  ...
  function renderShotDiffCard(reserveEl, diffRows) {
    ...
    const rows = (diffRows || [])
      .map(
        (row) =>
          `<div class="workflow-shot-diff-row"><span>${row.label}</span><strong>${row.value}</strong></div>`,
      )
      .join("");
  ...
  `;
    ...
    const historyBtn = reserveEl.querySelector(
  ...
  function renderShotAnalysis(normalized) {
    ...
    function integFlow(fromIdx, toIdx) {
      let vol = 0;
      for (
        let i = Math.max(1, fromIdx);
        i <= toIdx && i < elapsed.length;
        i++
      ) {
    ...
    if (hasPhases) {
      ...
      rows.push({
        label: t("analysis.preinfusionWater"),
        value: `${volPre.toFixed(1)} ml`,
      });
      rows.push({
        label: t("analysis.extractionWater"),
        value: `${volEx.toFixed(1)} ml`,
      });
      ...
      if (avgP != null)
        rows.push({
          label: t("analysis.avgPressure"),
          value: `${avgP.toFixed(1)} bar`,
        });
      if (avgF != null)
        rows.push({
          label: t("analysis.avgFlow"),
          value: `${avgF.toFixed(1)} ml/s`,
        });
    } else {
      const volTotal = integFlow(0, lastIdx);
      const allIdx = new Set(
        Array.from({ length: elapsed.length }, (_, i) => i),
      );
      const avgP = avgInSet(pressure, allIdx, 0.5);
      const avgF = avgInSet(flow, allIdx, 0.1);
      rows.push({
        label: t("analysis.totalWater"),
        value: `${volTotal.toFixed(1)} ml`,
      });
      if (avgP != null)
        rows.push({
          label: t("analysis.avgPressure"),
  ...
  function _buildPhaseSegments(shot, normalized) {
    const elapsed = Array.isArray(normalized?.elapsed)
      ? normalized.elapsed
      : [];
    if (!elapsed.length) return [];
    ...
    const phaseMarkers = Array.isArray(normalized?.phaseMarkers)
      ? normalized.phaseMarkers
      : [];
    ...
    const substates = Array.isArray(normalized?.substates)
      ? normalized.substates
      : [];
    ...
    for (let i = 1; i <= substates.length; i++) {
      const changed =
        i === substates.length || substates[i] !== substates[runStart];
      if (!changed) continue;
      const rawLabel = substates[runStart] || "phase";
      const label =
        rawLabel === "preinfusion"
          ? "Fill"
          : rawLabel === "pouring"
            ? "Pressurize"
            : rawLabel;
      out.push({
        ...
        start: elapsed[runStart] ?? 0,
          ...
          elapsed[Math.max(runStart + 1, i - 1)] ?? elapsed[elapsed.length - 1],
      });
      runStart = i;
    }
    return out;
  ...
  function createChartOpts(
    ...
    height,
    visibility,
    tempRange = { min: 80, max: 100 },
    { showTempAxis = true, padding = [12, 12, 12, 12] } = {},
  ) {
  ...
  function renderShotStats(reserveEl, normalized, workflow) {
    ...
    const stats = [
      {
        label: t("chart.time"),
        value: duration != null ? duration.toFixed(1) + " s" : "—",
        color: "#ffffff",
      },
      {
        label: t("chart.bar"),
        value: fmt(avgFinite(normalized?.pressure), 1),
        color: CHART_COLORS.pressure,
      },
      {
        label: t("chart.mlps"),
        value: fmt(avgFinite(normalized?.flow), 1),
        color: CHART_COLORS.flow,
      },
      {
        label: t("chart.gps"),
        value: fmt(avgFinite(normalized?.scaleRate), 1),
        color: CHART_COLORS.weightRate,
      },
      {
        label: t("chart.temp"),
        value: fmt(avgFinite(normalized?.temperature), 1),
        color: CHART_COLORS.temperature,
      },
    ];

    const actualWeight = integrateScaleRate(
      normalized?.elapsed,
      normalized?.scaleRate,
    );
    ...
    const CX = 60,
      ...
      DIAM = R * 2;
    const progress =
      actualWeight != null && targetWeight != null && targetWeight > 0
  ...
  function initLiveShotChart(graphEl) {
    ...
    try {
      ...
      const handleLiveLegendToggle = (seriesKey) => {
        const currentVisibility = getSeriesVisibility(
          graphEl._seriesVisibility,
          "live",
        );
      ...
      if (graphEl.id === "espresso-fs-graph") {
        renderEspressoFullscreenLegend(
          graphEl,
          visibility,
          handleLiveLegendToggle,
        );
  ...
  function updateLiveShotChart(graphEl, liveShot) {
    ...
    try {
      ...
      graphEl._chart.setData([
        ...
        liveShot.flow,
        liveShot.targetFlow,
        liveShot.smoothedScaleRate?.length
          ? liveShot.smoothedScaleRate
          : liveShot.scaleRate,
      ...
      if (graphEl._seriesVisibility?.steps !== false) {
        const now = Date.now();
        const phaseCount = Array.isArray(liveShot?.phaseMarkers)
          ? liveShot.phaseMarkers.length
          : 0;
  ...
  function renderLiveShotStats(reserveEl, liveShot, weight, workflow) {
    ...
    const stats = [
      ...
      {
        label: t("chart.gps"),
        value: currentScaleRate != null ? fmt(currentScaleRate, 1) : "—",
        color: CHART_COLORS.weightRate,
      },
      {
        label: t("chart.temp"),
        value: fmt(temperature, 1),
        color: CHART_COLORS.temperature,
      },
    ];
    ...
    const CX = 60,
      ...
      DIAM = R * 2;
    const progress =
      weight > 0 && targetYield != null && targetYield > 0
  ...
  function renderShotGraph(
    ...
    const visibility = getSeriesVisibility(
      seriesVisibility || graphEl._seriesVisibility,
      mode,
    );
    ...
    if (ref && Array.isArray(ref.elapsed) && ref.elapsed.length) {
      ...
      opts.series.push(
        {
          label: "Ref Pressure",
          stroke: _fadeHex(CHART_COLORS.pressure, A),
          width: 1.5,
          paths: refSpline,
          scale: "pressure",
          points: { show: false },
        ...
        {
          label: "Ref Flow",
          stroke: _fadeHex(CHART_COLORS.flow, A),
          width: 1.5,
          paths: refSpline,
          scale: "pressure",
          points: { show: false },
        ...
        {
          label: "Ref Scale",
          stroke: _fadeHex(CHART_COLORS.weightRate, A),
          width: 1.5,
          paths: refSpline,
          scale: "pressure",
          points: { show: false },
        ...
        {
          label: "Ref Temp",
          stroke: _fadeHex(CHART_COLORS.temperature, A),
          width: 1.5,
          paths: refSpline,
          scale: "temp",
          points: { show: false },
    ...
    try {
      ...
      cursorLine.style.cssText =
        "position:absolute;left:0;top:0;height:100%;border-right:1px dashed var(--c-label-3,#607D8B);pointer-events:none;display:none;will-change:transform;";
      ...
      function _showAtClientX(clientX) {
        ...
        const px = Math.max(
          0,
          Math.min((clientX - rect.left) / scaleX, over.offsetWidth),
        );
      ...
      let _touchStartX = 0,
        _touchStartY = 0,
        _touchMode = null; // null | 'scrub' | 'scroll'
  ...
  function updateRecipeRating(key, max, count) {
    document.querySelectorAll(".recipe-rating").forEach((el) => {
      if (el.dataset.recipeKey === key)
        el.outerHTML = _renderRecipeRating(key, max, count);
    });
  }

  let _onRecipesRendered = null;
  function setOnRecipesRendered(fn) {
    _onRecipesRendered = fn;
  }
    ...
    return shots
      ...
      <span class="history-shot-cell history-shot-cell--recipe">
        ...
        const rating =
          ...
          (shot.rating != null ? shot.rating : null);
        const ratingDisplay =
          rating != null
            ? _starRatingHtml(rating)
            : '<span class="history-shot-value">—</span>';
        const isFav = ann.extras?.favorite ?? shot.metadata?.favorite === true;
        const date = _historyFormatDate(shot.timestamp);
        const tags = Array.isArray(ann.extras?.tags)
          ? ann.extras.tags
          : Array.isArray(shot.metadata?.tags)
            ? shot.metadata.tags
            : [];
        const tagsHtml =
          tags.length > 0
  ...
  function updateHistoryShotDuration(shotId, seconds) {
    const el = document.querySelector(
      `#history-accordion-list .history-shot-duration[data-shot-id="${shotId}"]`,
    );
    if (el)
      el.textContent = Number.isFinite(seconds)
        ? `${seconds.toFixed(0)}s`
        : "—";
  }
  ...
  window.NSXUI = {
    ...
    setWaterDisplayUnit: (unit) => {
      _waterDisplayUnit = unit;
      setWaterLevel(waterLevelMm);
    },
    showToast,
    showStateToast,
